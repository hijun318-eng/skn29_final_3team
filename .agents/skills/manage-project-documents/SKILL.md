...
1. Run `git rev-parse --show-toplevel`, `git status --short`, and `git branch --show-current`.
...
9. Run `python .agents/skills/manage-project-documents/scripts/check_document_policy.py <changed paths>` and `git diff --check`.
