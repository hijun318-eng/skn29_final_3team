...
1. 단일 Git 정책에 매핑된 개인 branch 중 사용자가 요청한 source 목록과 각 worktree root를 기록한다. 사용자가 이름을 지정하지 않았다면 현재 branch가 매핑된 경우에만 source 하나로 사용하고, 아니면 중단한다.
...
5. 모든 source 병합 후 보고서 Skill의 post-merge mode를 각 source의 merge 직전·직후 SHA로 순서대로 적용한다. 반환된 `docs/markdown/daily_reports/team_summaries/` path만 한 번 stage하고 staged diff를 검토해, 비어 있지 않으면 보고 전용 commit 하나를 만든다.
