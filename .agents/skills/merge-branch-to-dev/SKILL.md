...
3. `dev`로 전환해 origin을 fetch하고 `git pull --ff-only origin dev`를 실행한 뒤 preflight script를 `--phase dev --session`으로 실행한다. session의 remote-only 여부는 자동으로 유지되며, 이 단계가 병합 직전 dev SHA를 같은 session에 기록한다.
...
8. 각 source worktree가 해당 branch에 있고 clean하며 `origin/<branch>`가 `origin/dev`의 조상인지 확인한다. 모두 맞으면 source worktree에서 `git merge --ff-only origin/dev`, `git push origin <branch>`를 실행하고 local source·`origin/<branch>`·`origin/dev`가 같은 commit인지 확인한다. remote-only이거나 하나라도 조건이 맞지 않으면 해당 source를 변경하지 않고 동기화가 생략된 이유를 보고한다.
