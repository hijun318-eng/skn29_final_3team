...
description: >-
  Update execution WBS and schedule views for verified mapped-task changes. Use for "update the WBS", "WBS 업데이트", or "일정·진척·담당자·산출물·근거 반영". Exclude no-impact, read-only/report-only, Git integration, and speculative updates.
---
...
5. task 행, 날짜 또는 상태가 바뀌면 영향받은 모든 view인 실행 WBS, phase 요약과 전체 count, 8주 일정, Mermaid Gantt, deliverable 일정을 동기화한다.
6. WBS에는 별도 작업 로그나 변경 내역을 추가하지 않고 Git 이력을 사용한다.
