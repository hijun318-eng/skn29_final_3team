"""보고서 인쇄/PDF 렌더링용 순수 SVG 차트 생성 모듈.

[핵심 목적]
외부 자바스크립트 차트 라이브러리(Chart.js 등) 없이 서버 사이드에서:
1. 선형(Line) 및 막대(Bar) 차트의 반응형 SVG 벡터 그래픽 생성
2. 다중 계열(Multi-series) 단위 일치성 검증 및 축 라벨 포맷팅
3. 최대 12개 데이터 포인트 균등 샘플링 및 범례(Legend) HTML 조립
을 완벽하게 처리합니다.
"""
...
from app.services.report.types import ReportDocumentRenderError
...
def _chart_svg(
    ...
    """아티팩트 데이터로부터 인쇄/PDF 호환 SVG 차트 및 범례 HTML 문자열을 생성합니다."""
