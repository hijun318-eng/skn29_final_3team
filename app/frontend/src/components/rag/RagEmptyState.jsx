/** 내부 문서 검색의 업무 영역과 질문 예시를 초기 화면에서 안내한다. */
...
/** 업무 영역은 안내하고 예시 질문만 채팅 입력으로 전달한다. */
export default function RagEmptyState({ onAsk, onBack, initialView = "" }) {
  const [view, setView] = useState(initialView);
  const askInternalGuideline = (question) => onAsk(`[내부 지침] ${question}`);
  return (
    <section className="chat-empty-state rag-empty-state">
      ...
      <div className="rag-help-actions">
        <button type="button" onClick={() => setView("documents")}>문서 목록 보기</button>
        <button type="button" onClick={() => setView("examples")}>질문 예시 보기</button>
      ...
      {view === "examples" && (
        <div className="rag-help-panel rag-example-list">
          {EXAMPLES.map(([label, question]) => (
            <button key={label} type="button" onClick={() => askInternalGuideline(question)}>
      ...
      {!view && (
        <div className="rag-recommended">
          ...
          {RECOMMENDED.map((question) => (
            <button key={question} type="button" onClick={() => askInternalGuideline(question)}>{question}</button>
          ))}
        </div>
      )}
    </section>
