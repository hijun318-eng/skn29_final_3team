...
export const ReportDocumentView = memo(function ReportDocumentView({
  ...
  const handleSaveHtml = useCallback(() => {
    if (approved && finalDocument) {
      onOpenFinalAsset("html", true);
      return;
    }
    const reportRoot = document.querySelector('[data-report-render-root="screen-preview"]')
      || document.querySelector('.report-page-canvas')
      || document.querySelector('.generated-preview');
    if (!reportRoot) return;
    const styles = Array.from(document.querySelectorAll('style, link[rel="stylesheet"]'))
      .map((el) => el.outerHTML)
      .join("\n");
    const title = selectedDefinition.title || "2026년 8월 호텔 운영 월간보고서";
    const blob = new Blob([
      `<!DOCTYPE html>
<html lang="ko">
<head>
...
${styles}
<style>
body { margin: 0; padding: 24px; background: #f1f5f9; font-family: Pretendard, -apple-system, BlinkMacSystemFont, system-ui, Roboto, sans-serif; }
.generated-preview, .report-page-canvas { margin: 0 auto; }
.legacy-document-actions, .report-finalization-panel, .report-preview-meta-strip { display: none !important; }
@media print {
...
${reportRoot.outerHTML}
  ...
  }, [approved, finalDocument, onOpenFinalAsset, selectedDefinition.title]);

  const handleSavePdf = useCallback(() => {
    if (approved && finalDocument) {
      onOpenFinalAsset("pdf", true);
      return;
    }
    window.print();
  }, [approved, finalDocument, onOpenFinalAsset]);
  ...
  return (
    <div ref={rootRef} className="page-content legacy-report-document generated-preview" data-report-render-root="screen-preview">
      <div className="legacy-document-actions">
        ...
        <div>
          <button type="button" onClick={handleSaveHtml} title="HTML 파일로 저장"><Download size={14} />HTML 저장</button>
          <button type="button" onClick={handleSavePdf} title="PDF 인쇄 및 파일로 저장"><Printer size={14} />PDF 저장</button>
          {!archived && <button onClick={onReturnToEditor} disabled={Boolean(pending)}><ArrowLeft size={14} />{approved ? "새 버전으로 편집" : "편집으로 돌아가기"}</button>}
          {!archived && isAdmin && approved && <button onClick={onRun} disabled={Boolean(pending)}><Send size={14} />최신 데이터로 다시 생성</button>}
          <div style={{ display: "none" }}>
            {currencyControl}
            <div className="report-orientation-switch" role="group" aria-label="읽기 전용 A4 용지 방향">
              <button type="button" aria-pressed={orientation === "landscape"} disabled><Maximize2 size={14} />가로</button>
              <button type="button" aria-pressed={orientation === "portrait"} disabled><Minimize2 size={14} />세로</button>
            </div>
            {presentation}
            {fullscreenSupported && (
      ...
      {archived && <section className="report-archive-readonly" role="status"><Archive size={17} aria-hidden="true" /><div><b>삭제된 보고서 · 읽기 전용</b><span>확정 문서는 계속 열람할 수 있습니다. 변경하거나 다시 실행하려면 휴지통에서 먼저 복원해 주세요.</span></div></section>}
      <div style={{ display: "none" }}>
