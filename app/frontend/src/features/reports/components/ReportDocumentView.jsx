  ...
  return (
    <div className="page-content legacy-report-document generated-preview" data-report-render-root="screen-preview">
      <div className="legacy-document-actions">
        ...
        <div><button onClick={onReturnToEditor} disabled={Boolean(pending)}><ArrowLeft size={14} />{approved ? "새 버전으로 편집" : "편집으로 돌아가기"}</button>{isAdmin && approved && <button onClick={onRun} disabled={Boolean(pending)}><Send size={14} />최신 데이터로 다시 생성</button>}</div>
      </div>
      ...
      {!approved ? <section className="report-finalization-panel" aria-labelledby="report-finalization-title">
        ...
        <div className="report-finalization-action">
          ...
          {isAdmin ? <button type="button" className="primary" onClick={onApprove} disabled={Boolean(pending) || isDirty}>{pending === "approve" ? <LoaderCircle className="spin" size={15} aria-hidden="true" /> : <LockKeyhole size={15} aria-hidden="true" />}{pending === "approve" ? "확정 중" : "보고서 확정"}</button> : <small>보고서 확정은 관리자에게 요청해 주세요.</small>}
        </div>
