...
export const ReportEditorToolbar = memo(function ReportEditorToolbar({
  ...
  return <header className="editor-topbar notion-editor-topbar">
    ...
    <div className="editor-command-actions">
      ...
      {builderV2 && onToggleTheme && <ThemeToggle className="report-editor-theme-toggle" theme={theme} onToggle={onToggleTheme} />}
      <button title={isDirty ? "변경사항을 먼저 저장해 주세요" : "저장된 보고서 미리보기"} onClick={onPreview} disabled={Boolean(pending) || isDirty}><Eye size={14} aria-hidden="true" />미리보기</button>
