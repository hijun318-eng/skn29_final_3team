...
export const ReportEditorToolbar = memo(function ReportEditorToolbar({
  ...
  return <header className="editor-topbar notion-editor-topbar">
    <div><button onClick={onLeave} disabled={Boolean(pending)}><ArrowLeft size={14} aria-hidden="true" />보고서</button><span className="notion-status-chip">{reportStatusLabel(selectedDefinition?.status)} · v{selectedDefinition?.version}</span></div>
    <div>
      ...
      <button title={isDirty ? "변경사항을 먼저 저장해 주세요" : "저장된 HTML 초안 확인"} onClick={onPreview} disabled={Boolean(pending) || isDirty}><Eye size={14} aria-hidden="true" />HTML 초안 확인</button>
