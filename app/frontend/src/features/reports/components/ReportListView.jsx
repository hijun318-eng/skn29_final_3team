...
export const ReportListView = memo(function ReportListView({
  ...
  return (
    <div className="page-content enterprise-reports-list">
      ...
      {definitionState === "empty" && !createOpen && <section className="report-empty-state"><span><FilePlus2 size={24} /></span><small>첫 보고서</small><h2>아직 작성한 보고서가 없습니다</h2><p>새 보고서를 만들면 편집 화면으로 바로 이동합니다.</p><button type="button" className="primary" onClick={() => setCreateOpen(true)}><FilePlus2 size={15} />첫 보고서 만들기</button></section>}
      {definitionState === "error" && <p className="report-api-state error"><ShieldAlert size={17} />보고서 목록을 불러오지 못했습니다.</p>}
      {definitionState === "ready" && <section className="card legacy-report-list"><div className="legacy-report-row legacy-report-head"><span>상태</span><span>버전·제목</span><span>구성</span><span>확정 시각</span><span>동작</span></div>{visibleDefinitions.map((definition) => <article className="legacy-report-row" key={`${definition.definitionId}-${definition.version}`}><strong>{reportStatusLabel(definition.status)}</strong><b>버전 {definition.version}<small>{definition.title}</small></b><span>{definition.blocks.length}개 블록</span><span>{definition.approvedAt ? formatSeoulTime(definition.approvedAt) : "—"}</span><nav className="legacy-report-actions" aria-label={`${definition.title} 동작`}><button className="edit" disabled={Boolean(pending)} onClick={() => onEdit(definition)}>{definition.status === "approved" ? "새 버전으로 편집" : "편집"}</button><button className="view" disabled={Boolean(pending)} onClick={() => onOpen(definition)}>{definition.status === "approved" ? "열기" : "미리보기"} <ChevronRight size={13} /></button></nav></article>)}</section>}
