...
export function reportApiError(error: unknown): string {
  if (error instanceof ReportApiError) {
    ...
    if (error.status === 409) {
      if (error.code === "REPORT_REVISION_CONFLICT") {
        return "다른 변경 사항이 먼저 저장되었습니다. 최신 상태를 다시 불러와 주세요.";
      }
      return error.message && error.message !== "Report API 요청에 실패했습니다."
        ? error.message
        : "다른 변경 사항이 먼저 저장되었습니다. 최신 상태를 다시 불러와 주세요.";
    }
