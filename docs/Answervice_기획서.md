...
**Answervice는 호텔의 여러 시스템에 흩어진 데이터를 자연어로 안전하게 분석하고, 출처가 확인된 결과를 반복 가능한 보고서로 만드는 내부 업무 서비스다.**

정식 제품명은 `Answervice`다. 2026-07-24 이전 문서의 `SensePlace`는 범위 전환 전 구형 명칭이며 현재 제품을 뜻하지 않는다.

### 1.2 핵심 가치

| 사용자의 현재 문제 | Answervice가 제공하는 가치 |
|---|---|
| 객실·고객·판매 데이터가 서로 다른 시스템에 있다. | 필요한 데이터를 한곳에서 찾고 승인된 관계로 함께 조회한다. |
| 현업 사용자가 데이터 위치와 SQL 문법을 알아야 한다. | 사용자는 업무 용어로 질문하고 시스템이 읽기 전용 분석을 수행한다. |
| 결과의 계산 조건과 출처를 다시 확인하기 어렵다. | 지표·기간·필터·데이터 출처를 결과와 함께 제공한다. |
| 같은 분석을 보고서에 반복해서 옮겨야 한다. | 검증된 표·차트를 출처가 보존된 보고서 블록으로 재사용한다. |

이 서비스의 핵심은 단순히 AI가 답변하는 것이 아니다. **답변에 사용한 데이터와 조건을 확인할 수 있고, 같은 결과를 보고서에서 다시 사용할 수 있다는 점**이 핵심 가치다.

### 1.3 실행 범위와 우선순위

| 우선순위 | 사용자가 받는 기능 | 이번 프로젝트의 완료선 |
|---:|---|---|
| P0 | 자연어 분석, 권한·안전성 검사, 표·차트·출처, 보고서 전달과 수동 실행 | 대표 질문이 실제 데이터 조회 흐름을 끝까지 통과하거나 안전하게 중단됨 |
| P1 | 5개 데이터 source의 카탈로그·연결 상태와 승인 정보 관리 | 각 source의 위치·소유자·연결 상태와 조회 경로를 확인할 수 있음 |
| P2 후속 | MCP, 사내 문서 RAG, ML-as-a-Tool, 외부 Report 배포 | I5 이후 별도 승인 전에는 구현하지 않음 |
| 선택 후속 | 고객 360 | 공통키·마스킹·권한·감사가 준비된 뒤 별도 일정으로 검토 |

워커힐 운영 환경은 실제 데이터가 아닌 합성 데이터로 모사하며, 특정 기업 성과를 주장하기 위한 사례가 아니라 이기종 데이터 연결 구조를 검증하기 위한 사례다.

### 1.4 성공의 정의

DataHub 설치나 목업 화면의 존재만으로 성공으로 판단하지 않는다. 사용자가 질문한 뒤 검증된 결과를 확인하고 보고서에 재사용하는 전체 흐름이 실제로 연결돼야 한다.
...
1. 정상 질문은 올바른 결과·표·차트·출처를 제공한다.
2. 기간이나 지표가 부족하면 임의로 답하지 않고 추가 조건을 요청한다.
3. 권한이 없거나 위험한 요청은 데이터 조회 전에 중단한다.
4. 데이터 source가 실패하면 성공한 것처럼 표시하지 않고 재시도 가능 여부를 안내한다.
5. 질문부터 조회·분석 결과·보고서까지 같은 요청을 추적할 수 있다.
...
| 확인할 질문 | 본 기획의 답 |
|---|---|
| 다른 AI 채팅 서비스와 무엇이 다른가? | 답변에 사용한 데이터·기간·조건을 확인할 수 있고, 검증된 결과를 보고서에서 다시 사용할 수 있다. |
| DataHub는 왜 필요한가? | AI가 사용할 데이터의 위치·구조·소유자·출처를 확인하는 기준 시스템으로 사용한다. |
| Trino는 왜 필요한가? | 여러 데이터 source를 원본 이동 없이 읽기 전용으로 조회한다. |
| AI가 모든 것을 판단하는가? | 아니다. AI는 질문 정리·조회문 후보·설명을 담당하고, 권한·실행·결과 통과 여부는 고정 규칙이 판단한다. |
| 어떤 AI 모델을 사용하는가? | Base 모델 후보를 같은 평가 조건으로 비교한 뒤 선정하며 LoRA는 명확한 개선이 있을 때만 적용한다. |
| 일정상 가장 큰 위험은 무엇인가? | 전체 실행 환경의 자원 경합, 실제 E2E 연결, 평가 질문과 정답 검수다. |
| 일정과 담당은 어디서 확인하는가? | 5인 역할과 2026-07-29~09-03 확정 실행 일정은 `02_WBS.md`에서 관리한다. |
| P2 기능도 이번에 만드는가? | 아니다. MCP·문서 RAG·ML-as-a-Tool과 고객 360은 별도 승인 전 구현하지 않는다. |
