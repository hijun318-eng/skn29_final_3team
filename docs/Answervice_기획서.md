...
| 문서 분류 | 일반 문서 |
| 버전 | v1.3 |
| 문서 기준일 | 2026-08-03 11:06 |
| 작성·수정 | 송민지 / Codex 반영 |

> 문서 상태: 최종 기획서
> 문서 버전: v1.3 — 승인자용 요약을 사용자 문제·서비스 흐름·완료선 중심으로 정제
> 작성 기준일: 2026-08-03
...
| 용어 | 의미 |
|---|---|
| **Context** | 질문에 답하기 위해 필요한 데이터, 지표, 연결 관계, 권한 정보를 한 묶음으로 정리한 것 |
| **G1·G2·G3** | 세 단계 검사: G1(데이터 근거 확인) → G2(안전한 SQL인지 확인) → G3(결과가 정확한지 확인) |
| **검증 결과물** | 질문 + 사용한 데이터 + 실행 조건 + 검사 결과를 한 묶음으로 보관한 것. 보고서에서 재사용 가능 |
| **소스 추적** | 질문부터 조회, 검사, 결과, 보고서까지 전 과정의 기록이 남아있어서 언제든 확인 가능 |
| **버전 관리** | 누가, 언제, 어떤 버전을 사용했는지 기록해두어 결과를 다시 재현할 수 있는 방식 |
| **기본 모델 vs 추가 학습** | 기본 모델로 시작한 후, 필요하면 추가로 최적화해서 성능을 개선하는 방식 |
...
**Answervice는 여러 부서의 데이터를 자연어 질문으로 안전하게 분석하고, 검증된 결과를 반복 가능한 보고서로 만드는 기업 내부 서비스다.**

- **사용자**: "이번 달 핵심 지표가 얼마예요?"라고 자연어로 질문
- **서비스**: 여러 부서 데이터를 조회해 표·차트·출처를 제공
- **장점**: 검증된 분석을 보고서에서 다시 사용 가능

### 1.2 핵심 가치 — 3가지 문제 해결

**문제 1: 데이터가 여러 시스템에 분산됨**
- 현재: 데이터를 찾으려면 여러 부서를 거쳐야 함
- 해결: DataHub에서 필요한 데이터를 한곳에서 검색

**문제 2: 질문에 답하려면 SQL 작성 능력이 필요함**
- 현재: 개발자가 아니면 조회 불가
- 해결: 자연어로 질문하면 시스템이 자동으로 조회 실행

**문제 3: 분석 결과의 근거를 보고서에서 다시 확인하기 어려움**
- 현재: 수치만 보고서에 옮겨 조건과 출처가 사라짐
- 해결: 검증된 결과와 근거를 함께 보존해서 반복 사용 가능

→ **핵심**: AI만 쓰는 게 아니라, **검증 + 추적 + 재사용**이 함께 되어야 신뢰할 수 있는 분석

### 1.3 구현 범위

**P0 핵심 기능: 질문부터 보고서까지 전체 흐름이 실제로 동작해야 함**
1. 사용자가 자연어로 질문
2. 권한·안전성 검사 
3. 데이터 조회 및 표·차트 생성
4. 검증된 결과를 보고서에 포함 및 실행

**P1 추가 기능: 데이터 관리와 카탈로그** (P0 안정화 후)
- 5개 데이터 원천의 위치·소유자·상태 확인
- 새 데이터 연결 시 자동 검색 가능

**P2·선택 기능: 향후 확장** (이번 프로젝트 범위 제외)
- 사내 문서 검색
- 예측 모델 연동
- 고객별 분석

**데이터**: 실제 기업 데이터가 아닌 합성 데이터 사용 (재현 가능성 확보)

### 1.4 성공의 정의

**목업이나 설치 완료가 아니라 end-to-end 동작이 성공의 기준**

| 성공 조건 | 의미 |
|---|---|
| ✓ 정상 질문 | 올바른 데이터를 표·차트·출처와 함께 제공 |
| ✓ 모호한 질문 | 임의로 답하지 않고 필요한 정보를 다시 물음 |
| ✓ 위험한 요청 | 조회 전에 차단하고 안전한 사유를 제공 |
| ✓ 실패 처리 | 일부 실패 시 성공한 것처럼 표시하지 않음 |
| ✓ 전체 추적 | 질문 → 데이터 선택 → 조회 → 검증 → 보고서까지 기록 유지 |

**예시: 보고서 재사용**
- 지난달 분석을 보고서에 포함 → 이달에 자동 재실행 → 결과와 데이터 출처가 모두 최신 상태 유지

### 1.5 핵심 기술

| 기술 | 역할 | 왜 필요한가 |
|---|---|---|
| **데이터 메타데이터 시스템** | 데이터를 찾고 분류하기 | AI가 어떤 데이터를 쓸 수 있는지 알아야 함 |
| **안전한 조회 엔진** | 여러 부서 데이터를 안전하게 조회 | 원본 데이터는 건드리지 않고 읽기만 함 |
| **AI 질문 처리** | 자연어를 데이터 조회로 변환 | 비개발자도 질문할 수 있게 함 |
| **검증 규칙** | 생성된 조회가 안전한지 확인 | 잘못된 조회는 실행 전에 차단 |
| **보고서 자동화** | 검증된 결과를 반복 실행 | 같은 분석을 매달 자동으로 재실행 |

### 1.6 자주 받는 질문

| Q | A |
|---|---|
| **다른 AI 서비스와 차이점?** | 답변의 근거(어떤 데이터, 어떤 조건)를 함께 제공하고, 검증된 결과를 보고서에서 재사용 가능 |
| **모든 판단을 AI가?** | 아니다. AI는 질문 해석과 조회 제안만 담당. 최종 실행과 승인은 고정 규칙과 관리자가 함 |
| **어떤 모델?** | 기본 모델을 먼저 테스트해서 명확한 개선이 있을 때만 최적화 |
| **비용과 성능?** | 합성 데이터로 테스트해서 필요한 규모를 미리 파악. 불필요한 조회는 차단해서 비용 절감 |
| **실제로 동작하나?** | 대표 질문 3~5개로 end-to-end 테스트. 설치 완료가 아니라 **실제 분석 흐름 완성**이 성공 기준 |
| **일정과 진행 상황?** | `02_WBS.md`에서 5인 역할과 8월 말 기준의 구체적 일정 확인 가능 |
...
**문제 1: 분석에 걸리는 시간**
- 데이터 위치, SQL, 권한을 모르면 누군가에게 물어봐야 함
- 여러 부서를 거쳐야 해서 답변까지 며칠이 걸림

**문제 2: 반복 보고서의 번거로움**
- 같은 분석을 매달 반복하려면 처음부터 다시
- 조건과 근거가 시간이 지나면 명확하지 않음

→ **해결책**: 질문 1번 → 검증 → 보고서 → 다음달 자동 재실행

### 2.2 현재 상황

| 단계 | 현재 | 문제 |
|---|---|---|
| **데이터 찾기** | "어디 데이터 봤어?" | 담당자에게 매번 물어봐야 함 |
| **SQL 작성** | 개발자가 직접 작성 | 비개발자는 못함 |
| **권한 확인** | 각 시스템마다 다름 | 실수로 접근 제한 데이터 조회 가능 |
| **결과 보고** | 숫자만 보고서에 옮김 | 조건과 출처가 사라짐 |
| **다음달 반복** | 처음부터 다시 | 자동화 불가능 |

### 2.3 Answervice의 작동 방식

```
1️⃣ 사용자가 질문
   ↓
2️⃣ 시스템이 데이터 목록에서 관련 데이터 검색
   ↓
3️⃣ AI가 자동으로 조회문 생성
   ↓
4️⃣ 자동 검사: 권한 있나? 안전한가? 데이터가 있나?
   ↓
5️⃣ 검사 통과한 조회만 실행
   ↓
6️⃣ 결과가 정확한지 최종 확인
   ↓
7️⃣ 표·차트·근거와 함께 사용자에게 제시
   ↓
8️⃣ 사용자가 "이 결과를 보고서에 담아"라고 하면 보존
   ↓
9️⃣ 다음달에 자동 재실행 (질문, 조건, 근거 모두 유지)
```
...
| 목표 | 성공 기준 |
|---|---|
| **자연어 질문으로 분석** | "이번 달 매출은?" → 2초 내 표·차트·출처 제공 |
| **안전한 데이터 접근** | 위험한 요청은 자동 차단. 조회 기록 남음 |
| **여러 부서 데이터 연결** | 5개 시스템의 데이터를 원본 이동 없이 조회 |
| **자동 보고서** | 검증된 분석을 매달 자동으로 재실행 |
| **추적 가능한 결과** | "이 수치는 언제, 어디서 나왔나?" 언제든 확인 |

### 3.2 이번에 구현할 것 / 나중에 할 것

**P0 필수 (이번에 반드시 완료)**
- 질문 → 데이터 조회 → 표·차트 → 보고서 까지 실제 동작
- 보고서 저장 및 매달 자동 재실행

**P1 추가 (필요하면)**
- 5개 데이터 원천의 위치·상태 확인
- 새로운 데이터 원천 추가 시 자동 검색 가능
...
| 사용자 | 하는 일 | 필요한 것 |
|---|---|---|
| **일반 직원** | "이번 달 매출이 얼마야?" 질문 → 결과 이해 | 표, 차트, 조건 설명, 근거 |
| **관리자** | 정기 보고서 만들고 매달 자동 실행 | 어떤 데이터를 썼는지, 언제 마지막으로 실행했는지 |
| **데이터 담당자** | 데이터 원천 관리, 권한 설정, 시스템 감시 | 어디서 뭘 조회했는지 기록 |

### 4.2 사용 예시 A — 분석 질문

**상황:** 마케팅 담당자가 “지난달 VIP 고객의 구매액이 얼마야?” 질문

**흐름:**
1. 자연어 질문 입력
2. 시스템이 자동으로 필요한 데이터 검색
3. 안전성·권한·정확성 검사
4. 표·차트·조건 설명과 함께 결과 제시
5. “이 결과를 보고서에 담아” → 출처와 함께 저장

**성공 기준:** 
- 표에 보인 수치 = 실제 데이터셋 + 적용된 조건과 일치
- 다음 달에도 같은 조건으로 자동 재실행 가능

### 4.3 사용 예시 B — “왜” 질문 (P2 - 향후)

**상황:** “지난달 매출이 떨어진 이유가 뭘까?” 질문

**흐름:**
1. 데이터: “매출이 20% 감소했습니다”
2. 정책 문서 검색: “8월 1일 프로모션 종료”
3. 답변: “데이터상 감소 관측 + 정책 변경 기록 + 관련성 제시”

**중요:** 데이터와 문서를 분리해서 제시 (하나만으로 결론 내리지 않음)
...
2. 이번주 데이터로 자동 재실행
3. 결과 검토 후 보고서 작성
4. 매주 같은 시간에 자동 실행 설정

**중요:** 
- 오래된 분석을 다시 깨끗이 만들 필요 없음
- 한 블록이 실패해도 나머지는 계속 실행

### 4.5 사용 예시 D — 새 데이터 원천 추가

**상황:** 새로운 데이터 시스템이 추가됨

**흐름:**
1. 새 시스템을 Answervice에 등록
2. 자동으로 데이터 목록 수집
3. “VIP 고객” 질문을 다시 실행
4. 새 시스템의 데이터도 함께 조회 가능 확인

**중요:** 화면에 보이는 것만으로 끝나지 않고, 실제로 조회에 사용되고 출처에 표시되어야 함
...
**P0 필수:**
- 챗: 질문 → 데이터 검색 → 검사 → 표·차트·출처 제시
- 보고서: 결과 저장 → 수동 실행 → 스케줄 실행

**P1 추가:**
- 데이터 카탈로그: 5개 원천의 상태·위치·소유자 확인
...
| 상황 | 예시 | 시스템의 대응 |
|---|---|---|
| **질문이 모호함** | "매출" (기간·고객·부서 명시 없음) | "다음 조건을 알려줘: 기간, 고객군" 재질문 |
| **권한 없음** | 접근 제한 데이터 요청 | 실행 전 차단 + 안전한 사유만 표시 |
| **데이터 문제** | 원천 시스템 오류 | 어느 시스템에서 실패했는지, 재시도 가능한지 알림 |
| **부분 성공** | 5개 시스템 중 3개만 조회됨 | 성공한 것만 결과로 표시, 실패 부분 명시 |
| **근거 부족** | 데이터는 있지만 설명 문서 없음 | 단정하지 않음 + 추가 확인 필요 표시 |

**원칙:** 기술 에러는 숨기고, 사용자가 수정할 수 있는 정보만 제시

---

## 6. 전후 비교

| 업무 | 현재 | Answervice 사용 후 | 주의사항 |
|---|---|---|---|
| **데이터 찾기** | 담당자에게 매번 물음 | 자동 검색 | 소유자의 권한 확인은 여전히 필요 |
| **질문하기** | 개발자가 SQL 작성 | 자연어 질문 | 시스템이 생성한 SQL은 항상 검증됨 |
| **데이터 연결** | 수작업으로 수동 JOIN | 승인된 연결 규칙만 사용 | 새로운 연결은 따로 검토·승인 |
| **결과 확인** | 숫자만 봄 | 숫자 + 조건 + 출처 | 최종 판단은 담당자 책임 |
| **보고서 작성** | 매달 처음부터 | 기존 결과 재사용·재실행 | 승인 후에야 저장됨 |

---

## 7. 시스템 구조 개요

Answervice는 **순차적 검증 파이프라인** 방식으로 작동합니다:

1. **질문 이해** → AI가 자연어를 구조화된 조건으로 변환
2. **데이터 맥락 구성** → 사용 가능한 데이터와 권한 확인
3. **안전성 검사** → 자동으로 생성된 SQL이 안전한지 확인
4. **실행 및 검증** → 조회 실행 후 결과가 정확한지 확인
5. **결과 제시** → 표·차트·설명과 함께 근거 제공
6. **보고서 저장** → 재사용 가능한 형태로 보관

**핵심 특징:** AI만 쓰는 게 아니라, **3단계 자동 검사**를 거쳐야 실행됨
...
Answervice는 3단계 자동 검사를 거쳐 데이터 조회를 실행합니다:
1. **G1**: 데이터 근거 확인 (필요한 정보 있는지?)
2. **G2**: SQL 안전성 확인 (위험한 조회는 아닌지?)
3. **G3**: 결과 정확성 확인 (수치가 맞는지?)

각 단계에서 문제가 발견되면 실행 전에 중단합니다.

### 개발팀 참고 자료

- LLM Node 3개 (질문 해석 → SQL 생성 → 설명)
- Gate 3개 (Context, Policy, Result 검증)
- DataHub: 메타데이터 관리
- Trino: 안전한 읽기 전용 조회
- 캐싱 및 버전 관리로 성능 최적화

더 자세한 아키텍처는 별도의 기술 문서 참고
|---|---|---|---|---|
...
아래 표의 **`현재 구현`** 열은 2026-07-31 기준 `app/frontend/package.json`과 실제 소스 트리에서 확인한 값이다. 계획과 구현이 다른 항목은 어느 쪽으로 수렴할지 I1 Contract Freeze에서 R1이 판정한다. 계획값을 확정 사실로 인용하지 않는다.
