...
이 문서는 Walkerhill 고정 v4 후보를 전제로 한 2026-08-13 계획이며 현재 release·데이터·런타임 지시로 사용하지 않는다. 현행 기준은 저장소 루트 [`AGENTS.md`](../../../AGENTS.md), [`docs/product/`](../../product/), [`docs/architecture/`](../../architecture/)다. 과거 `production-integrity-audit.md`는 삭제됐고, 현행 무결성 근거는 `scripts/audit_repository_integrity.py` 실행 결과다.

# Answervice v4 통합 개발 계획 (역사 자료)
...
| 항목 | 현재 확인 내용 | 근거 |
|---|---|---|
| V4.3 데이터 release | 현재 실행 입력은 versioned SQL release와 checksum manifest다. 구형 33 CSV 생성기 계약은 제거됐다. | [V4.3 manifest](../../../infrastructure/database/releases/walkerhill_v4_3_20260815_derived_1/manifest.json) |
| 데이터·메타데이터 Gate | DataHub와 Trino의 실제 자산·필드 집합, native governance, schema fingerprint를 동적으로 다시 읽어 일치해야 한다. 과거 정적 PASS 파일은 증거로 사용하지 않는다. | [동적 DataHub verifier](../../../infrastructure/database/datahub/verify_semantic_catalog.py) |
| 애플리케이션 cutover | V4.3 SQL release는 존재하지만 DataHub가 51개 Dataset·578개 필드를 모두 read-back하기 전에는 cutover 완료로 판정하지 않는다. | [V4.3 품질 보고서](../../../infrastructure/database/releases/walkerhill_v4_3_20260815_derived_1/00_문서/품질_보고서.md) |
...
| Node 2 일반화 경계 | Node 2는 질문별 SQL 족보가 아니라 DataHub에서 읽은 schema context·metric rules·join graph·time rules·query policy만 소비해야 한다. | [Node I/O 계약](../../../src/ai/contracts/node_io.v0.1.json), [prompt registry](../../../src/ai/prompt_registry.py) |
