# Answervice — 호텔 VOC·운영 지원 플랫폼 WBS

| 항목 | 내용 |
|---|---|
| 문서 설명 | Answervice의 실행 작업, 담당, 상태, 일정, 산출물과 요구사항 추적 관계를 관리하는 WBS 작업본 |
| 문서 분류 | 산출물 작업본 |
| 버전 | v4.9 |
| 문서 기준일 | 2026-07-29 10:42 |
| 작성·수정 | 김재홍 |
| 산출물 번호 | 02 |
| 제출 일자 | 2026-07-16 |
...
> 공식 서비스명 **Answervice** · Baseline·실험·Gate의 일정과 상태를 관리한다.
...
- 제출용 변환 파일은 현재 편집 범위에서 제외하며 일정·상태의 단일 기준으로 사용하지 않는다.
...
7. 작업 종료 시 문서 하단 **WBS 작업 로그** 맨 위에 일시, 실행 WBS ID, 변경 요약, 결과 구분을 한 줄로 기록한다(검증 세부는 commit·PR에 남기고 로그에는 핵심만).
...
|---|---|---|---|---|---|---|---|---|---|
| `1.5` | Answervice 기획·요구사항·WBS 범위 정합화 | `DOC-003` | 전체 Baseline | 박준희·송민지 | 07/13 | 07/24 | 검토 | Answervice 결정 적용 | 범위·완료 기준 |
| `1.6` | 화면 설계 제출·후속 정합화 보류 | `DOC-013` | 화면 요구사항 | 송민지 | 07/13 | 07/24 | 진행 | 없음 | 화면 구조·상태 기준 |
| `1.7` | 데이터 항목·시간/grain·권한·8 intent(핵심 4종 P0 우선 E2E: 기간 KPI·전주 비교·시간대 도착·근거 조회)·평가 분리·LLM 모델 최소 합의 | `DOC-003`, `DOC-010` | 전체 Baseline | 박준희·김재홍·정승·윤대성 | 07/27 | 07/28 | 검토 | Answervice 기획 | 데이터·연동 경계 기준 |
...
| `6.1`~`6.4`, `6.9`, `6.12`~`6.14` | 기능 A 로그인·권한·job·plan·SQL Guard·차트 연결 | `DOC-014` | `REQ-F-002`,`003`, `REQ-NF-001` | 김재홍·윤대성·송민지 | 07/28 | 08/12 | 대기 | `1.7` 최소 합의 | 내부 연동 경계 |
...
| `7.1` | 반례 세트 v2 21건 Baseline Gate evidence | `DOC-016` | Gate 차단 Baseline | 전원 | 08/13 | 08/14 | 대기 | 기능 A~D 완료 | 미생성 |
| `7.6` | 반례 Gate 자동 채점·후속 회귀 결과와 결함 조율 | `DOC-016` | `TST-001`, `TST-003` | 박준희·윤대성·김재홍 | 08/13 | 08/28 | 대기 | 기능 A~D 완료·Gate 판정 | 미생성 |
...
Baseline 43개는 최종 구현 범위이며 5주차 Gate 차단 항목과 동의어가 아니다. 기능 A~D 실행 경로와 최소 진실성·보안·회귀 기준은 8/12까지 연결해 8/13~14에 검증한다. `UI-004`의 단위·기간 표시는 Gate 차단이고 색상 외 구분만 비차단이다. `NFR-001`, `OPS-001`의 최종 검증도 Gate 비차단으로 7주차까지 수행하며, `TST-003`은 5주차 Gate 자동 채점·6~7주차 회귀·8주차 Golden Path 5회로 관리한다.
...
| 프론트 | · | 목업 fixture | 6종 중간발표 목업 | UI-001~003 UI-004 단위·기간 UI-005 RPT-001~002 기능 A~D 연결 | 회귀 | UI-004 색상 외 구분 최종 검증(비차단)·선택 확장 |

> 이 표는 최소 구현과 Gate 연계 시점을 요약한다. 최종 검증 종료일과 담당·현황의 단일 기준은 실행 WBS이며, 8주차에는 TST-003 Golden Path 5회와 전원 발표·시연·소스 정리를 수행한다. ID는 공용 추적 기준을 적용한다.
...
5. Gate의 차단 기준은 기능 A~D 실행 경로와 최소 진실성·보안·자동 채점이다. `UI-004`의 단위·기간 표시는 Gate에서 확인하고 색상 외 구분, `NFR-001`, `OPS-001`의 최종 검증은 비차단으로 7주차까지 수행한다.
...
- **5주차 위험:** 기능 A~D 연결과 Gate가 같은 주라 여유가 작다. 4주차까지 단위 모듈을 준비하고 5주차에는 연결·회귀만 허용한다. Gate 차단 결함이 남으면 `7.4` 선택 확장을 자동 취소한다.
...
| 1.3 | 요구사항 및 Answervice 현재 범위 확정·제출 | 요구사항 정의서 | 박준희·송민지 | 검토 | 07/10 | 07/24 | **07/16** |
...
| 1.7★ | A~D 기능·최소 테이블·평점 우선 조회·원본/학습 분리·권한·8 intent·평가·LLM 모델 최소 합의 | 요구사항·연동 기준·백엔드 경계 합의 기록 | 박준희·김재홍·정승·윤대성 | 검토 | 07/27 | 07/28 |  |

### 데이터 수집·전처리·DB

| ID | 작업 항목 | 산출물 | 담당 | 현황 | 시작 | 마감 | 제출일 |
|---|---|---|:--:|:--:|:--:|:--:|:--:|
| 2.1 | 합성 VOC 원본·한국어 번역·학습 파생 schema·label·template 설계안 | 합성 VOC 생성·lineage 기준안 | 정승·윤대성 | 대기 | 07/21 | 07/24 |  |
| 2.2 | 규칙 label+승인 모델 문장 다양화 합성 VOC v0 생성 | 합성 VOC | 정승 | 대기 | 07/29 | 07/29 |  |
| 2.3 | 장소별 매출·운영비·예약·체크인·체크아웃·인력 단일 운영 데이터 v0 생성 | 합성 운영 데이터 | 정승 | 대기 | 07/29 | 07/29 |  |
...
| 2.7 | PostgreSQL 최소 업무 테이블 6개·모니터링 view 설계 후 validation 통과분 적재 | DB 스키마·적재본 | 정승 | 대기 | 07/31 | 08/05 |  |
| 2.8 | 원본/학습 lineage·직원/알림·호텔 문서·read-only 흐름 문서화 | 데이터베이스/저장소 설계 문서 | 김재홍·정승 | 진행 | 07/29 | 07/31 | **07/31** |
| 2.9★ | `metric_catalog`·role scope·장소/담당자 매핑 초안 설계(`1.7`에서 검토·확정) | semantic catalog 초안 | 김재홍·정승 | 대기 | 07/21 | 07/27 |  |
...
| 5.2 | 기능 A~D 공용 평점·매출·운영수익·예약·투숙·인력 최소 KPI 계산 | KPI 모듈 | 정승 | 대기 | 08/07 | 08/10 |  |
| 5.3★ | 적재 후 analytical Gate 구현(bucket 완전성·집계 정합) | 분석 품질 Gate | 정승·김재홍 | 대기 | 08/07 | 08/10 |  |
| 5.4★ | 평점·운영 교차분석, 추가 질의 저점 리뷰, 해결 가능성·관측·원인 후보·반대 근거 구성 | 교차분석·근거 모듈 | 윤대성·정승·박준희 | 대기 | 08/11 | 08/11 |  |
...
| 5.9★ | batch 적재 완료→analytical Gate `READY`→감지→담당자 알림 trigger | 트리거·알림 모듈 | 김재홍 | 대기 | 08/11 | 08/11 |  |
...
| 6.2 | Django `/api/v1/*` job·polling·scope·audit·report decision·알림/조치 API | 업무 API | 김재홍 | 대기 | 07/31 | 08/04 |  |
| 6.3★ | 기능 B~D Incident·리포트·해결 가능성·담당자 알림·모니터링 view 통합 및 수용 시나리오 검증 | Agent 기능·Incident 브리프 | 윤대성·김재홍·박준희 | 대기 | 08/12 | 08/12 |  |
| 6.4 | 기능 A 평점 우선 질문·추가 저점 리뷰·최소 표·차트 연결 | 대화형 분석 결과 | 송민지·김재홍 | 대기 | 08/07 | 08/12 |  |
...
| 6.8 | Answervice 배포 구성·최종 시연 환경·fallback·기술 스택 시각화 | 시스템 구성도 | 김재홍 | 대기 | 08/18 | 08/21 | **08/21** |
...
| 6.13★ | 기능 A 결과 유형별 최소 표·차트·단위·기간과 D stale 상태 표시 연결 | query·모니터링 UI | 송민지 | 대기 | 08/07 | 08/12 |  |
...
| ID | 작업 항목 | 산출물 | 담당 | 현황 | 시작 | 마감 | 제출일 |
|---|---|---|:--:|:--:|:--:|:--:|:--:|
| 7.1★ | 기능 A~D 반례 세트 v2 21건·intent 역할 matrix·복수 seed Baseline Gate | Gate 판정·통합 evidence | 전원 | 대기 | 08/13 | 08/14 |  |
...
> Answervice 동기화: 기능 A~D와 Gate 필수 secret 점검은 8/12, 반례 세트 v2 21건 Gate는 8/13~14, 연휴 기본 비배치 뒤 보완은 8/18부터다. ML/DL·VectorDB·sLLM·다역할 Agent·편차 점검은 독립 실험이고 일반 추천 질문과 외부 전달은 팀이 필요성을 합의하기 전까지 보류한다.
...
| ID | 세부 업무 | 담당 | 시작 | 마감 | 우선순위 | 관련 요구사항 | 산출물 |
|---|---|:--:|:--:|:--:|:--:|---|---|
| PV-2.1 | 합성 VOC 원본·번역·학습 파생 schema·label·template 설계안 | 정승·윤대성 | 07/21 | 07/24 | 🔴Must | DAT-001,DAT-002,DAT-005,DAT-008 | 수집 데이터 보고서 |
| PV-2.2 | A~D 데이터 항목·최소 테이블·metric catalog·LLM 모델 최소 합의 | 박준희·김재홍·정승·윤대성 | 07/27 | 07/28 | 🔴Must | DAT-003,DAT-007,DAT-008,INT-001 | 요구사항·연동 기준 합의 기록 |
| PV-2.3 | 단일 운영·VOC·직원·알림·문서 합성 데이터 생성·scenario 주입 | 정승 | 07/29 | 08/05 | 🔴Must | DAT-003,DAT-008,SEC-001 | 합성 데이터 |
| PV-2.4 | 적재 전 정규화·schema·개인정보성 값 validation | 정승·김재홍 | 07/30 | 07/31 | 🔴Must | FUN-005,SEC-001 | 데이터 전처리 결과서 |
| PV-2.5 | PostgreSQL 최소 업무 테이블·모니터링 view 구축·validation 통과분 적재 | 정승·김재홍 | 07/31 | 08/05 | 🔴Must | DAT-001~005,DAT-007,DAT-008 | DB/저장소 설계 문서 |
...
| ID | 세부 업무 | 담당 | 시작 | 마감 | 우선순위 | 관련 요구사항 | 산출물 |
|---|---|:--:|:--:|:--:|:--:|---|---|
| PV-4.1 | VOC 평점 통계·증감률(코드) | 정승 | 08/03 | 08/07 | 🔴Must | AI-005 | LLM 활용 소프트웨어 |
| PV-4.2 | 매출·운영수익·예약·투숙·인력 기간 비교(코드) | 정승 | 08/03 | 08/07 | 🔴Must | AI-007 | LLM 활용 소프트웨어 |
| PV-4.3 | 평점·운영 교차분석·저점 리뷰·해결 가능성·관측·반대 근거·부족 데이터 | 윤대성·정승·박준희 | 08/11 | 08/11 | 🔴Must | AI-008,AI-009,BIZ-002,BIZ-003,NFR-003,NFR-005 | 교차분석·근거 모듈 |
...
| PV-5.3 | 기능 A 평점 우선·추가 저점 리뷰 포함 보장 질문 8종 의도 해석·semantic plan 품질 | 윤대성·김재홍 | 07/31 | 08/03 | 🔴Must | FUN-006,AI-010 | query plan 모듈 |
...
| PV-5.8 | 기능 A 최소 표·차트와 D 모니터링 stale 상태 연결 | 송민지 | 08/07 | 08/12 | 🔴Must | UI-005,OPS-001 | 웹 애플리케이션 |
...
| PV-6.2 | Django `/api/v1/*` job·polling·audit·report decision·알림/조치 | 김재홍 | 07/31 | 08/04 | 🔴Must | REQ-F-003,REQ-F-004,REQ-F-007,SEC-003,OPS-001 | 웹 애플리케이션 |
...
| ID | 세부 업무 | 담당 | 시작 | 마감 | 우선순위 | 관련 요구사항 | 산출물 |
|---|---|:--:|:--:|:--:|:--:|---|---|
| PV-7.1 | 운영 홈·기능 A 진입·기능 D 장소별 최신 상태 | 송민지 | 08/07 | 08/11 | 🔴Must | UI-001,OPS-001 | 웹 애플리케이션 |
...
| ID | 세부 업무 | 담당 | 시작 | 마감 | 우선순위 | 관련 요구사항 | 산출물 |
|---|---|:--:|:--:|:--:|:--:|---|---|
| PV-9.1 | 기능 A~D 반례 세트 v2 21건 Gate·5회 최종 재현 | 전원 | 08/13 | 09/02 | 🔴Must | TST-001,TST-003 | 서비스 테스트 결과 보고서 |
...
최신 기록을 위에 추가한다. 저장소 변경이 없는 단순 조사·설명은 기록하지 않는다. 열은 일시·WBS ID·변경 요약·결과 구분 4개로 유지한다.

| 일시(KST) | WBS ID | 변경 요약 | 결과 구분 |
|---|---|---|---|
| 2026-07-29 10:42 | 2.8 | 데이터베이스/저장소 설계 제출본 `docs/deliverables/05_데이터베이스저장소설계서_29기_3팀.xlsx` 등록 및 XLSX 구조 검증 근거 연결 | 설계 산출물 |
| 2026-07-28 16:11 | 1.5 | 공식 프로젝트명을 Answervice로 확정하고 README·화면·구조도·활성 문서의 브랜드 표기를 동기화 | 기획 계약 |
| 2026-07-28 15:18 | 1.6 | 최종 기획 v1.0을 기준으로 제품 목표·P0/P1/P2 IA·Guarded Text-to-SQL 상태·15개 화면·12-column 보고서·DataHub/Context/Trino·운영 감사 명세를 `05_화면설계서.md` v5.0에 반영하고 활성 요구사항 충돌을 기록 | 설계 산출물 |
| 2026-07-28 15:11 | 1.6 | 데이터 카탈로그의 통합 구조·연결 현황·제품 탐색 화면 명세와 합성 fixture 경계, 반응형·QA 근거를 `05_화면설계서.md` v4.1에 동기화 | 설계 산출물 |
| 2026-07-28 14:16 | 1.6 | `05_화면설계서.md`를 현재 React 목업과 활성 요구사항에 맞춰 Baseline·확장 경계, IA, 화면 상세, 상태·접근성·추적성 기준으로 전면 재작성 | 설계 산출물 |
| 2026-07-27 17:44 | 2.8 | `06_데이터베이스저장소설계서.md`에 데이터 허브 24개 엔터티의 논리·물리 명세, ERD, 무결성·적용 전략 초안 작성 | 설계 산출물 |
| 2026-07-23 12:07 | 1.3, 1.4, 1.5 | 대응 템플릿 참조는 유지하고 다른 문서의 이름·경로·참고 과정 표현 제거 | 공식 산출물 정리 |
| 2026-07-23 11:53 | 1.3, 1.5, 1.7, 2.1, 2.3, 2.7~2.9, 5.2, 5.4, 5.9, 6.2~6.4, 6.13 | A~D 기능, 최소 업무 테이블, 원본/학습 리뷰 분리, 평점 우선·저점 리뷰 추가 질의, 담당자 알림·모니터링 범위 정합화 | 기획 계약 |
| 2026-07-23 11:26 | 1.4, 8.2 | OpenWiki를 로컬 OAuth provider로 초기화하고 저장소 위키 5개와 credential·외부 전송 운영 경계 생성 | 개발 지원 |
| 2026-07-23 11:08 | 1.4, 8.2 | OpenWiki 수동 갱신 workflow와 자동 비용·외부 전송 방지 경계 명시 | 개발 지원 |
| 2026-07-23 10:14 | 1.4, 1.6, 1.7, 8.2 | 공식 산출물과 보조 자료를 분리하고 실행·검증 기준 정합화 | 문서 운영 |
| 2026-07-22 16:50 | 1.4, 8.2 | 근거 우선순위·보호 범위·검증 조건과 실행 메타데이터 정비 | 문서 운영 |
| 2026-07-22 16:31 | 1.4, 8.2 | 산출물 01~21 상세 규격의 적용 조건·확인 순서·충돌 우선순위 정리 | 문서 운영 |
| 2026-07-22 16:16 | 1.4, 8.2 | staged Markdown 정책 자동 검사 연결 | 검증 자동화 |
| 2026-07-22 16:06 | 1.4, 8.2 | AI 에이전트 지침 축약과 WBS·산출물 규격 절차 분리 | 문서 운영 |
| 2026-07-22 15:51 | 1.4, 8.2 | 활성 기준·보호 자료·기술 스택 계획 표현과 검증 분류 정비 | 문서 운영 |
| 2026-07-22 15:08 | 1.4, 1.5 | 화면 기준·작성자·변경 이력·자동 검증 범위 정합화 | 문서 운영 |
| 2026-07-22 10:39 | 1.5 | 공통 파일 관리 헤더와 기본 정보 표 분리 | 기획 산출물 |
| 2026-07-22 10:27 | 1.3, 1.4, 1.5 | 번호형 산출물 3건의 표준 헤더·기준일·변경 내역 형식 정비 | 기획 산출물 |
| 2026-07-22 09:19 | 1.4, 8.2 | 실행 위치·제품 중립 호출·보고 검증·Git preflight 보강 | 개발 지원 |
| 2026-07-21 17:32 | 1.4, 8.2 | 문서 관리·보고 갱신·branch 통합 절차와 읽기 전용 검증 추가 | 개발 지원 |
| 2026-07-21 | 1.7, 2.9, 5.3, 5.4, 5.9, 6.3, 6.7, 6.9 | WISE 기준의 사실·수치·시맨틱 관계·질의·Trigger·Agent·AI Guide 경계와 수용 기준 정리 | 기획 계약 |
| 2026-07-21 | 1.4 | 실행 WBS 63개와 공식 산출물 21건+옵션 1건, 상태·진척률·38일 평일 간트 구성 | WBS 산출물 |
| 2026-07-21 | 8.2 | HTML preview를 local-only로 분류하고 보호 영역 변경 금지 | 저장소 운영 |
| 2026-07-21 | 1.4, 8.2 | WBS 제목·메타데이터·산출물 번호·제출일·최근 변경 내역 정비 | WBS 산출물 |
| 2026-07-21 | 1.7, 2.9, 5.7, 6.9, 6.14 | 백엔드 상태·계약 충돌과 FastAPI 내부·외부 연동 경계 정리 | 연동 계약 |
| 2026-07-21 | 1.7, 3.7, 6.3, 7.1 | 반례·Incident 식별자·intent P0 우선순위 등 마감 보완 4건 반영 | 기획 계약 |
| 2026-07-21 | 1.3, 1.5, 1.7, 6.x, 7.x | 공식 제목·감성 P0·Gate·VOC schema·상태·감사·API·Incident 경계 정리 | 기획 계약 |
| 2026-07-21 | 1.3, 1.5, 3.6, 6.x, 7.x, 8.4 | Baseline 43개·Gate·회귀·pgvector·GraphDB 미도입 범위 정합화 | 기획 계약 |
| 2026-07-21 | 1.7, 4.3~8.5 | 발표 전담과 B·D 공통구간 지원을 포함한 R&R 재배분 | 역할 계약 |
| 2026-07-21 | 1.3~8.5 | 서비스 범위·최소 데이터·연동·반례·평가·역할·8주 일정과 ID 체계 정합화 | 기획 계약 |
| 2026-07-20 | 1.3, 1.5, 1.7 | 단일 호텔·전량 합성·기능 A/B·인증·8 intent·Gate·독립 실험·최종 재현 기준 통합 | 기획 계약 |
| 2026-07-20 | 1.5, 1.6 | 관리자 의사결정 중심 정보 우선순위·MVP Golden Path·화면·서비스 책임 확정 | 기획 계약 |
| 2026-07-20 | 1.4, 8.2, 9.1 | WBS 단일 관리·작업 로그·보고·branch 병합·보호 영역·공용 작업 지침 정리 | 운영 체계 |

> 2026-07-20 이전의 개별 편집 단위 기록은 위 항목으로 통합했다.

## 변경 내역

| 버전 | 일시 | 요약 |
|---|---|---|
| v4.9 | 2026-07-29 10:42 | WBS 2.8의 일정·담당·진행 상태는 유지하고 데이터베이스/저장소 설계 XLSX 제출본 등록 및 구조 검증 근거 연결 |
| v4.8 | 2026-07-28 16:11 | WBS 1.5의 일정·담당·상태는 유지하고 공식 프로젝트명을 Answervice로 변경해 활성 화면·문서·구조도 표기를 동기화 |
| v4.7 | 2026-07-28 15:18 | WBS 1.6 일정·담당·진행 상태는 유지하고 최종 기획 기반 화면설계서 v5.0의 전면 개편 근거와 활성 요구사항 동기화 필요 기록 |
| v4.6 | 2026-07-28 15:11 | WBS 1.6 일정·담당·진행 상태는 유지하고 데이터 카탈로그 최신 화면 명세·QA 근거 연결 |
| v4.5 | 2026-07-28 14:16 | WBS 1.6 일정·담당·진행 상태는 유지하고 화면설계서 전면 개편 근거와 작업 이력 연결 |
| v4.4 | 2026-07-27 17:44 | WBS 2.8을 진행으로 전환하고 데이터 허브 기반 데이터베이스/저장소 설계 초안 근거 연결 |
| v4.3 | 2026-07-23 12:07 | 대응 템플릿 참조는 유지하고 다른 문서의 이름·경로와 참고·기반 작성 과정 표현 제거 |
| v4.2 | 2026-07-23 11:53 | 기존 일정·담당·상태를 유지하고 A~D 기능·최소 데이터 구조·리뷰 lineage·알림·모니터링 작업 범위와 기획 추적 관점 정합화 |
| v4.1 | 2026-07-23 11:35 | `dev` 최신 WBS 이력을 보존하면서 ChatGPT/Codex OAuth 기반 OpenWiki 초기화와 생성 위키 근거를 통합 |
| v4.0 | 2026-07-23 10:14 | 실행 기준을 유지하면서 보조 자료 분류와 검증 지침 정합화 |
| v3.9 | 2026-07-22 17:18 | 제출 산출물·프론트엔드 목업 변경을 최신 기준에 반영 |
| v3.8 | 2026-07-22 | WISE 벤치마킹 기준과 실행 WBS·작업 로그 정합화 |
| v3.7 | 2026-07-22 16:50 | AI 에이전트 지침 중복 축약과 WBS 실행 조건·검증 범위 정비 |
| v3.6 | 2026-07-22 16:31 | 산출물 01~21 상세 규격의 적용 조건 명문화 |
| v3.5 | 2026-07-22 16:16 | staged Markdown 문서 정책을 강제하는 pre-commit 검증 연결 |
| v3.4 | 2026-07-22 16:06 | AI 에이전트 상시 지침 축약과 WBS·산출물 규격 절차 분리 |
| v3.3 | 2026-07-22 15:51 | 기준 우선순위·기술 계획 표현과 검증 분류 호환성 보강 |
| v3.2 | 2026-07-22 15:08 | 헤더·관리 기준 정합화 기록을 시간순으로 병합 |
| v3.1 | 2026-07-22 10:39 | 공통 헤더와 기본 정보 표 분리 |
| v3.0 | 2026-07-22 10:27 | 번호형 산출물 작업본 3건의 표준 헤더·최근 변경 내역 형식 정리 |
| v2.9 | 2026-07-22 09:19 | 범용 실행 경로·검증 안전성과 AI 에이전트 지침 연결 보강 |
| v2.8 | 2026-07-21 17:32 | 프로젝트 반복 절차를 Skill 3종과 결정론적 검증 script로 분리 |
| v2.7 | 2026-07-21 15:50 | HTML preview의 local-only Git 제외 정책 반영 |
| v2.6 | 2026-07-21 15:44 | 표준 메타데이터·산출물 정보·제출 형식·최근 변경 내역 추가 |
