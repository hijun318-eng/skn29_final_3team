...
> 공식 서비스명 **SensePlace** · Baseline·실험·Gate의 일정과 상태를 관리한다.
...
|---|---|---|---|---|---|---|---|---|---|
| `1.5` | SensePlace 기획·요구사항·WBS 범위 정합화 | `DOC-003` | 전체 Baseline | 박준희·송민지 | 07/13 | 07/24 | 검토 | SensePlace 결정 적용 | 범위·완료 기준 |
| `1.6` | 화면 설계 제출·후속 정합화 보류 | `DOC-013` | 화면 요구사항 | 송민지 | 07/13 | 07/24 | 진행 | 없음 | 화면 구조·상태 기준 |
| `1.7` | 데이터 항목·시간/grain·권한·8 intent(핵심 4종 P0 우선 E2E: 기간 KPI·전주 비교·시간대 도착·근거 조회)·평가 분리·LLM 모델 최소 합의 | `DOC-003`, `DOC-010` | 전체 Baseline | 박준희·김재홍·정승·윤대성 | 07/27 | 07/28 | 검토 | SensePlace 기획 | 데이터·연동 경계 기준 |
...
> SensePlace 동기화: 기능 A~D와 Gate 필수 secret 점검은 8/12, 반례 세트 v2 21건 Gate는 8/13~14, 연휴 기본 비배치 뒤 보완은 8/18부터다. ML/DL·VectorDB·sLLM·다역할 Agent·편차 점검은 독립 실험이고 일반 추천 질문과 외부 전달은 팀이 필요성을 합의하기 전까지 보류한다.
