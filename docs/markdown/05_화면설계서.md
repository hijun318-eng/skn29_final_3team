...
| 항목 | 내용 |
|---|---|
| 문서 설명 | 최종 기획 v1.1, 요구사항정의서 v1.2와 활성 React 목업을 대조해 사용자 경험, IA, 화면·상태·권한·접근성 및 구현 경계를 확정한 화면설계 산출물 |
| 문서 분류 | 산출물 작업본 |
| 버전 | v6.2 |
| 문서 기준일 | 2026-07-30 13:55 |
| 작성·수정 | 송민지 |
| 산출물 번호 | 05 |
| 제출 일자 | 2026-07-24 |
| 대응 템플릿 | 없음 |
...
| 내용 기준 | `../Answervice_기획서.md` v1.1, `01_요구사항정의서.md` v1.2, `02_WBS.md` v6.8 |
...
| 단계적 확장 | P2 Tool·RAG·ML 및 고객 360은 I5 이후 비차단 후속 단계이며, R1의 별도 일정·계약·비용·보안 Gate 승인 전 route와 메뉴를 노출하지 않는다. |
...
| P2 후속 | MCP·문서 RAG·ML-as-a-Tool·외부 Report 배포 | I5 이후 R1이 별도 일정·계약·비용·보안 Gate와 실행 묶음을 승인하기 전 route·메뉴·worker·dependency 비활성 |
| 후속 | 고객 360 | I5 이후 identity mapping·masking·전용 권한·감사·별도 수용 기준 승인 전 route와 메뉴 비활성 |
...
| `후속 대기` | 현재 목업이 있어도 I5 이후 R1 후속 Gate 승인 전 route·메뉴·구현 계약에서 제외함 |
| `구현 차이` | 확정 목표 설계와 현재 정적 목업의 route·상태·데이터 계약이 다름 |

### 1.5 기준 문서 정합성과 적용 결정

`01_요구사항정의서.md` v1.2가 최종 기획 v1.1의 P0/P1과 I5 이후 후속 경계를 75개 영구 ID로 구조화했으므로 본 문서의 화면 범위와 수용 기준은 두 문서에 동기화한다. 실제 목업은 레이아웃·상호작용 참고 근거이며 구현 완료나 route 승인 근거가 아니다.
...
| P2·고객 360 | I5 이후 비차단 후속 | `/catalog/tools`, `/customers` 직접 접근 가능 | 후속 Gate 승인 전 메뉴·route 차단 후 목업 코드만 보존 |
...
| 후속 범위 사용자 | I5 이후 별도 Gate 승인 후 마스킹 고객 분석 | 고객 360 | 현재 범위에서는 접근 불가 |
...
│  └─ backup·restore 상태
└─ 후속 대기 route
   ├─ Tool 관리                                 I5 이후 후속 Gate 승인 전 차단
   └─ 고객 360                                  I5 이후 후속 Gate 승인 전 차단
```

별도 통합 관제, 현황 맵, 내부 리뷰 데이터, 독립 온톨로지 맵은 목표 IA에 포함하지 않는다. 현재 목업의 `/catalog/ontology`는 자산 상세의 lineage·Context 관계로 흡수한다. MCP Tool·고객 360은 I5 이후 별도 Gate 승인 전 메뉴와 직접 route를 모두 차단한다.
...
| 후속 대기 | `/catalog/tools` | 없음 | 정적 목업 | I5 이후 후속 Gate 승인 전 404/권한 거부, 메뉴 미노출 |
| 후속 대기 | `/customers` | 없음 | 있음 | I5 이후 후속 Gate 승인 전 404/권한 거부, 메뉴 미노출 |
...
현재 `/catalog/tools` 목업은 구현 참고 자료로만 보존한다. REQ-SCP-001에 따라 I5 이후 R1 후속 Gate 승인 전 메뉴·route·registry 변경을 활성화하지 않으며 공식 화면 ID를 부여하지 않는다.

### 7.13 후속 대기 목업 — 고객 360
...
| 계약 | 필요한 결정 |
|---|---|
| 요구사항 기준 | `01_요구사항정의서.md` v1.2의 75개 ID; R1 계약 검토 후 I1 version 고정 |
...
| 항목 | 확정 설계 | 구현 Gate |
|---|---|---|
| 프로젝트 공식 범위 | 최종 기획 v1.1·요구사항 v1.2의 P0/P1 | I1에서 계약 version 승인 |
| 서비스명·frontend | `Answervice`, `app/enterprise-react` 단일 활성 앱 | package·화면·배포 표기 회귀 확인 |
| Tool·고객 360 | I5 이후 비차단 후속 | R1의 별도 일정·계약·비용·보안 Gate와 실행 묶음 승인 전 route 차단 |
...
| 버전 | 일시 | 요약 |
|---|---|---|
| v6.2 | 2026-07-30 13:55 | dev 병합 시 최신 WBS v6.8을 기준으로 갱신하고, 기획 v1.1·요구사항 v1.2 및 I5 이후 비차단 후속 방향을 유지해 과거 v1.0 기준으로의 회귀를 방지 |
| v6.1 | 2026-07-30 11:55 | 기획 v1.1·요구사항 v1.2·WBS v6.0에 맞춰 P0/P1를 I5 완료선으로 유지하고 P2·고객 360을 I5 이후 비차단 후속 Gate로 동기화하며 기존 13개 화면 ID와 목업 QA 근거를 보존 |
