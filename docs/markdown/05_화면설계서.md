...
| 항목 | 내용 |
|---|---|
| 문서 설명 | 최종 기획 v1.1의 사용자 경험, 정보 구조, 화면 구성, 상태, 권한, 접근성 및 구현 경계를 정의한 화면설계 산출물 |
| 문서 분류 | 산출물 작업본 |
| 버전 | v5.2 |
| 문서 기준일 | 2026-07-30 09:32 |
| 작성·수정 | 송민지 / 3팀 사용자 요청·Codex 반영 |
| 산출물 번호 | 05 |
| 제출 일자 | 2026-07-24 |
| 대응 템플릿 | 없음 |
| 상태 | 최종 기획 반영본 · 공식 01·02·03 제출본 동기화 필요 |
| 프로젝트 적용 사례 | 워커힐 유사 합성 호텔 운영 환경의 분산 데이터 분석·자동 리포팅 |
| 기획 기준 | 최종 기획서 v1.1, 2026-07-30 |
| 구현 기준 | `../../app/enterprise-react/src` React 정적 목업 |
...
| 단계적 확장 | P2 Tool·RAG·ML 및 고객 360은 I5 이후 각 후속 Gate가 통과된 경우에만 메뉴를 노출한다. |
...
| P2 확장 | 데이터 카탈로그 내 Tool 관리, 운영 문서 RAG, ML-as-a-Tool | 각 Tool의 권한·버전·오류·평가 Gate를 별도로 통과함 |
| 후속 단계 | 고객 360 | I5 이후 identity mapping, masking, 전용 권한·감사 승인 후 별도 time-box로 구현함 |
...
| 상태 | 의미 |
|---|---|
| `목표 설계` | 최종 기획 v1.1에 따라 구현해야 하는 화면 또는 동작 |
...
| `P2 비활성` | 선행 Gate 전에는 route와 메뉴를 사용자에게 노출하지 않음 |
| `동기화 필요` | 활성 요구사항·WBS 또는 다른 공식 산출물과 최종 기획이 충돌함 |

### 1.5 공식 문서 충돌과 적용 결정

최종 기획서와 공식 Markdown WBS를 본 화면설계의 현재 제품·실행 기준으로 사용한다. 저장소에는 활성 `docs/markdown/01_요구사항정의서.md`가 없으며, 제출본 `docs/deliverables/01_요구사항정의서_29기_3팀.xlsx`, `docs/deliverables/02_WBS_29기_3팀.xlsx`, `docs/deliverables/03_프로젝트기획서_29기_3팀.docx`는 구형 SensePlace·VOC/Django 내용을 담고 있어 Answervice의 현재 근거로 사용할 수 없다. 다음 공식 제출 전 세 제출본의 내용 동기화가 필요하며, 동기화 전에도 현재 구현은 기획서 v1.1·WBS v5.5·본 화면설계를 따른다.

| 문서 | 확인된 상태 | 현재 적용 | 필요한 교정 |
|---|---|---|---|
| `docs/deliverables/01_요구사항정의서_29기_3팀.xlsx` | SensePlace·VOC/Django 요구사항 | 현재 기능·수용 기준의 원본으로 사용하지 않음 | Answervice 요구·추적 ID·수용 기준으로 공식 제출본 동기화 |
| `docs/deliverables/02_WBS_29기_3팀.xlsx` | SensePlace·VOC·8 intent 일정 | 현재 담당·상태·일정의 원본으로 사용하지 않음 | WBS v5.5의 99개 현재 태스크와 비차단 후속 4건으로 공식 제출본 동기화 |
| `docs/deliverables/03_프로젝트기획서_29기_3팀.docx` | SensePlace 프로젝트 기획 내용 | `docs/Answervice_기획서.md`를 현재 기획 원본으로 사용 | 기획서 v1.1 내용으로 공식 제출본 동기화 |
...
|---|---|---|---|
| 업무 실무자 `BUSINESS_USER` | 질문, 결과·근거 확인, 보고서 블록 전달 | 분석 챗, 허용 보고서 | 질문·후속 질문, 표·차트 탐색, `보고서에 담기` |
| 보고 관리자 `REPORT_MANAGER` | 정의 편집, 승인, 실행·스케줄 관리 | 보고서 전체, 분석 챗 | 블록 편집·승인, 수동 실행, Gate 통과 후 스케줄 활성화 |
| 데이터 관리자 `DATA_ADMIN` | 소스·자산·Context·정책 상태 확인 | 데이터 카탈로그, 운영·감사 | 연결 상태 조회, 자산 검색, Context release·정책 확인 |
| 시스템 관리자 `SYSTEM_ADMIN` | 요청 trace, 오류, 보존·백업 상태 확인 | 운영·감사, P2 Tool | request 추적, 기술 상세 열람, Tool 상태 변경 |
| 감사자 `AUDITOR` | 권한·정책·실행 증적 검토 | 운영·감사 | 읽기 전용 검색·내보내기 |
| 후속 사용자 `CUSTOMER_ANALYST` | 마스킹 고객의 횡단 데이터 분석 | 고객 360 | 후속 Gate에서 승인된 고객 프로필·활동·내부 분석 챗 열람 |

P0 역할의 원본은 versioned policy와 migration이다. 화면은 유효 role과 policy version을 읽기 전용으로 보여주며 사용자가 자신의 role을 바꾸는 기능은 제공하지 않는다.
...
별도 통합 관제, 현황 맵, 내부 리뷰 데이터, 독립 온톨로지 맵은 목표 IA에 포함하지 않는다. MCP Tool 관리는 ADMINISTRATION에 중복 메뉴를 만들지 않고 데이터 카탈로그의 P2 탭으로 노출한다.
...
| P2 | `/catalog/tools` | `SCR-TOOL-001` | 정적 목업 | P2 활성 시에만 Tool contract·health·audit |
| 후속 | `/customers` | `SCR-C360-001` | 있음 | I5 이후 별도 Gate 승인 후 고객 360 |
...
| 화면 ID | 화면명 | 사용자 | 우선순위 | 주요 기능 | 최종 기획 근거 |
|---|---|---|---|---|---|
| `SCR-AUTH-001` | 로그인·유효 역할 | 전체 | P0 지원 | 인증, 역할·policy version, 합성 환경 고지 | §17.1, §17.4 |
| `SCR-CHAT-001` | 분석 챗 | 실무자·관리자 | P0 | 질문, 진행 상태, 결과·근거, 후속 질문, 보고서 전달 | §4, §5.2, §15.2 |
| `SCR-RPT-001` | 보고서 목록 | 실무자·보고 관리자 | P0 | 일·주·월 정의, 상태, 최근 실행, 생성 | §11, §15.3 |
| `SCR-RPT-002` | 보고서 상세·미리보기 | 보고 관리자 | P0 | 정의 버전, 블록 미리보기, 승인, 수동 실행, 스케줄 | §11.1, §11.5~§11.7 |
| `SCR-RPT-003` | 12-column 블록 에디터 | 보고 관리자 | P0 | drag·resize, AI assistant, chat artifact 삽입, 저장 | §11.2~§11.4 |
| `SCR-RUN-001` | 보고서 실행 이력 | 보고 관리자 | P0 운영 | run 상태·`as_of`·정의 버전·블록 성공/실패 | §11.1, §11.5 |
| `SCR-RUN-002` | 보고서 실행 상세 | 보고 관리자·감사자 | P0 운영 | snapshot, checksum, source, 재실행, 부분 실패 | §11.1, §11.6 |
| `SCR-CAT-001` | 데이터 카탈로그 허브 | 데이터·시스템 관리자 | P1 | 5개 소스 상태, 자산 검색, 요약 지표 | §8.7, §15.4 |
| `SCR-AST-001` | 데이터 자산 상세 | 데이터 관리자 | P1 | URN, schema, owner, tag, lineage, Trino mapping | §8, §9.2 |
| `SCR-CONN-001` | 연결 상태 | 데이터·시스템 관리자 | P1 | engine, ingestion, catalog, owner, 오류 | §8.2~§8.5 |
| `SCR-CTX-001` | Context·정책 | 데이터 관리자 | P1 | metric, time, dimension, JOIN, release, approval | §9.2 |
| `SCR-AUD-001` | 운영·감사 검색 | 시스템 관리자·감사자 | P0 운영 | role·policy, request 검색, retention·backup | §15.4, §17 |
| `SCR-AUD-002` | 요청 trace 상세 | 시스템 관리자·감사자 | P0 운영 | context→model/policy→query→artifact/report | §7.5, §17.2 |
| `SCR-TOOL-001` | Tool 관리 | 데이터·시스템 관리자 | P2 | MCP/RAG/ML Tool contract·권한·health·audit | §12.6, §15.4 |
| `SCR-C360-001` | 고객 360 | 후속 사용자 | 후속 | 마스킹 프로필·활동·지표·내부 분석 챗 | §15.5, §19.3 |
...
RAG 결과는 문서명·버전·발효일·인용 위치를, ML 결과는 `모델 예측`·model version·feature set version·기준 시각을 표시한다. DataHub mutation Tool은 별도 승인 전 활성화하지 않는다.

### 7.13 `SCR-C360-001` 고객 360(후속)
...
프런트 직원의 고객 응대용 외부 챗, 자동 메시지, 자동 보상은 포함하지 않는다. 현재 MVP에서는 메뉴와 직접 route를 숨기고, I5 이후 후속 Gate 통과 시에만 노출한다.
...
| `CANCELLED` | 사용자 또는 정책에 의해 실행 취소 | 취소 시각·범위를 확인하고 새 실행 |
...
| 기획 항목 | 적용 화면 | UI 수용 기준 |
|---|---|---|
| §4 사용자 시나리오 | 분석 챗, 보고서, 카탈로그 | 질문→근거→artifact→보고서, 소스 온보딩 흐름을 완료함 |
| §5 기능 우선순위 | 전체 IA | P0·P1과 I5 이후 P2·고객 360 메뉴를 단계별로 노출함 |
| §7·§9 실행 구조 | 분석 챗, 감사 | Controller 진행과 G1·G2·G3의 성공·중단을 재구성함 |
| §8 DataHub·Trino | 카탈로그, 자산, 연결 | URN·Trino FQN·실행 source가 일치함 |
| §11 보고서 | 보고서 전체 | definition/run 분리, 12-column, artifact 왕복, 부분 실패를 지원함 |
| §12 P2 RAG·MCP | Tool, 분석 근거 | SQL과 문서 evidence를 분리하고 version·권한을 표시함 |
| §13 ML Tool | Tool, 분석 근거 | 예측을 사실과 구분하고 model·feature version을 표시함 |
| §14 합성 데이터 | 전체 | `synthetic`, seed, schema version, 마스킹을 표시함 |
| §15 화면·디자인 | 전체 | 메뉴·상태·근거·접근성·반응형 원칙을 충족함 |
| §17 보안·감사 | 인증, 감사, 전체 | 역할·mask·trace·retention·backup 상태를 확인함 |
| §18 검증 | 전체 | 필수 30건이 올바른 결과 또는 안전한 중단으로 종료됨 |
...
5. Template·Plan Cache·Result Cache 경로 모두 G1·G2·G3와 entitlement 확인을 생략하지 않았음을 trace에서 확인한다.
...
|---|---|---|---|
| `/agent` | 대화, 분석 단계, KPI, 전략 카드, 데이터 제품 | 부분 정합 | Controller 상태, 표·차트·근거·모호성·artifact 중심으로 재구성 |
| `/reports` | 목록, drag and drop 편집, 문서 열람 상호작용 | 부분 정합 | 12-column, definition/run 분리, 승인·수동 실행·schedule Gate·서버 저장 |
| `/catalog` | 통합 흐름, 요약, 연결 현황, Explorer, 데이터 제품 | 부분 정합 | DataHub URN·Trino 단일 엔진·Context·실제 상태 계약 반영 |
| `/connections` | 정적 연결 카드·필터 | 부분 정합 | `/catalog/connections` 정보 구조와 ingestion·catalog 상태로 정리 |
...
| `/customers` | 고객 프로필·여정·선호·챗 | 후속 단계 | 현재 navigation과 직접 route에서 숨기고 I5 이후 identity·mask·권한·감사 승인 후 활성화 |
| 공통 shell | sidebar, sticky header, lazy page, 반응형 | 재사용 가능 | 새 IA, role, 합성 환경, 404, page error boundary 반영 |
...
8. OpenAPI 확정 후 TanStack Query로 서버 상태를 연결하고 local fixture fallback을 명시적으로 분리한다.
9. P2 Tool과 고객 360은 현재 I5 완료와 무관하게 비활성 상태로 두고, I5 이후 각 후속 Gate에서 feature flag와 서버 권한을 승인한다.

### 12.3 구현 전 선행 계약

| 계약 | 필요한 결정 |
|---|---|
| 요구사항 기준 | 기획서 v1.1·WBS v5.5를 현재 기준으로 사용하고, 구형 01·02·03 제출본을 다음 공식 제출 전에 범위·ID·수용 기준과 동기화 |
| 명칭 | 서비스 공식 명칭과 워드마크; 현재 후보를 확정 사실로 사용하지 않음 |
...
|---|---|---|---|
| production build | Vite build 성공 | Pass — 기존 목업 기준 2026-07-28 | 코드 변경 시 반복 |
| 현재 7개 route | 오류 없이 렌더링 | Pass — 기존 목업 기준 | 목표 route 추가 후 갱신 |
...
| 접근성 | keyboard, focus, 200%, chart alternative | Not Run | 화면별 자동·수동 QA |
| 반응형 | 1440·1024·768·360px | Partial — 기존 breakpoint만 존재 | 새 grid·drawer QA |
| P2·고객 360 Gate | I5 이후 후속 Gate 전 route·메뉴 미노출 | Fail — 현재 route 직접 접근 가능 | feature flag·server authz |

## 14. 결정 필요 항목

| 항목 | 현재 권고 | 결정 전 제한 |
|---|---|---|
| 공식 제출본 동기화 | 기획서 v1.1·WBS v5.5를 기준으로 구형 `01`·`02` XLSX와 `03` DOCX 내용을 교정 | 동기화 전 제출본을 현재 요구·일정·기획 근거로 사용하지 않음 |
| 서비스명 | `Answervice` | 화면·문서·패키지 표기를 공식 프로젝트명으로 통일 |
| Tool 메뉴 위치 | 데이터 카탈로그의 P2 탭으로 단일 노출 | ADMINISTRATION 중복 메뉴 금지 |
| 고객 360 | I5 이후 후속 time-box | 후속 Gate·feature flag·권한·감사 승인 전 메뉴와 직접 route 숨김 |
...
| 버전 | 일시 | 요약 |
|---|---|---|
| v5.2 | 2026-07-30 09:32 | 구형 공식 01·02·03 제출본 상태를 사실대로 교정하고, G2′ 단회 전이·Cache Gate·취소 상태·P2 및 고객 360 후속 노출 조건을 동기화 |
