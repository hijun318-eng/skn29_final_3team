...
| 항목 | 내용 |
|---|---|
| 문서 설명 | 최종 기획 v1.0의 사용자 경험, 정보 구조, 화면 구성, 상태, 권한, 접근성 및 구현 경계를 정의한 화면설계 산출물 |
| 문서 분류 | 산출물 작업본 |
| 버전 | v5.0 |
| 문서 기준일 | 2026-07-28 15:18 |
| 작성·수정 | 송민지 |
| 산출물 번호 | 05 |
| 제출 일자 | 2026-07-24 |
| 대응 템플릿 | 없음 |
| 상태 | 최종 기획 반영본 · 요구사항정의서 동기화 필요 |
| 프로젝트 적용 사례 | 워커힐 유사 합성 호텔 운영 환경의 분산 데이터 분석·자동 리포팅 |
| 기획 기준 | 최종 기획서 v1.0, 2026-07-28 |
...
| 단계적 확장 | P2 Tool·RAG·ML 및 고객 360은 선행 Gate가 통과된 경우에만 메뉴를 노출한다. |
...
| 선택 범위 | 고객 360 | identity mapping, masking, 전용 권한 승인 후 time-box로 결정함 |
...
최종 기획서는 본 화면설계의 최신 제품 목표다. 다만 저장소의 활성 `01_요구사항정의서.md`와 프로젝트 상시 지침은 아직 호텔 VOC·운영 이슈 분석 Baseline을 기준으로 하므로, 다음 항목은 팀 승인과 요구사항 개정 전까지 구현 완료로 간주하지 않는다.

| 항목 | 최종 기획 기준 | 현재 활성 요구사항 기준 | 본 문서 적용 | 필요한 후속 결정 |
|---|---|---|---|---|
| 핵심 문제 | 5개 사일로의 대화형 데이터 분석·자동 리포팅 | 단일 호텔 합성 VOC×운영 분석 | 목표 IA는 최종 기획 기준으로 교체 | 프로젝트 주제 문구와 `01` 전면 동기화 |
| 데이터 계층 | 4종 엔진·5개 소스, DataHub, Trino | 단일 PostgreSQL 중심 경량 카탈로그 | 카탈로그·커넥션·연합 근거를 목표 설계에 반영 | `DAT`, `INT`, `SEC`, `NFR` 요구사항 재정의 |
| Text-to-SQL | 런타임 Context, Trino SQL, G1·G2·G3 | 8개 intent와 결정론 SQL builder·Guard | 최종 기획의 Controller 상태를 기준으로 설계 | `FUN-006`, `AI-010`, `TST` 개정 |
| LLM 호출 | Node 1·2·2′·3, 경로별 최대 4회 | 단일 LLM 호출 중심 | 역할별 진행 상태와 감사 trace를 반영 | 모델 호출·평가 요구사항 개정 |
| 보고서 | 일·주·월, 12-column grid, 실행·스케줄 | 주간 HTML DRAFT 중심 | 일·주·월을 P0 운영 범위로 설계 | `RPT-001`~`002` 확장 |
| VOC·Customer 360 | VOC 별도 범위, Customer 360 선택 | VOC가 핵심, Customer 360 미승인 | VOC 전용 화면을 목표 IA에서 제거; 360은 선택 유지 | 프로젝트 고정 주제와의 충돌 승인 |
| Tool·RAG·ML | P2 | 대부분 승인 후 실험 | 선행 Gate 전 메뉴 비노출 | P2 요구사항·수용 기준 추가 |
...
| 선택 사용자 `CUSTOMER_ANALYST` | 마스킹 고객의 횡단 데이터 분석 | 고객 360 | 승인된 고객 프로필·활동·내부 분석 챗 열람 |
...
| 선택 | `/customers` | `SCR-C360-001` | 있음 | 별도 승인 후 고객 360 |
...
| `SCR-C360-001` | 고객 360 | 선택 사용자 | 선택 | 마스킹 프로필·활동·지표·내부 분석 챗 | §15.5, §19.3 |
...
프런트 직원의 고객 응대용 외부 챗, 자동 메시지, 자동 보상은 포함하지 않는다. MVP에서는 메뉴를 숨긴다.
...
| §5 기능 우선순위 | 전체 IA | P0·P1·P2·선택 메뉴를 단계별로 노출함 |
...
| 계약 | 필요한 결정 |
|---|---|
| 요구사항 기준 | 최종 기획과 `01_요구사항정의서.md`의 범위·ID·수용 기준 통합 |
...
| 항목 | 현재 권고 | 결정 전 제한 |
|---|---|---|
| 프로젝트 공식 범위 | 최종 기획을 기준으로 `01`과 상시 프로젝트 원칙을 동기화 | 상충 항목을 구현 완료·수용 완료로 보고하지 않음 |
| 서비스명 | 기획 내용이 드러나는 별도 명칭 확정 | `SensePlace`, `NEXUS`, `OctoPort`를 공식명으로 단정하지 않음 |
| Tool 메뉴 위치 | 데이터 카탈로그의 P2 탭으로 단일 노출 | ADMINISTRATION 중복 메뉴 금지 |
| 고객 360 | MVP 제외, 별도 time-box | feature flag·권한 승인 전 메뉴 숨김 |
