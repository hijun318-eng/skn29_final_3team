...
| DEPLOY-001 | 모델 배포 | 프론트 개발환경 | React+Tremor 작업용 로컬 디렉터리 구조와 Git 제외 기준 구성 | 송민지 | 🟢 DONE | P2 | 2026-07-20 | 2026-07-20 | 100 | PLAN-001 | 기능별 source 디렉터리가 로컬에 있고 `frontend/` 전체가 Git 추적에서 제외됨 | 디렉터리 존재, `git check-ignore frontend/README.md`, `git status`, `git diff --check` 확인 | `.gitignore` | 2026-07-20 15:44 | 로컬 `frontend/`는 개인 작업용이며 별도 결정 전 원격 저장소에 공유하지 않음 |
...
| OPS-010 | 프로젝트 운영 | 주간보고 연속 갱신 | 일일보고 작성 시 해당 프로젝트 주차의 주간보고를 유사 항목 통합 방식으로 자동 갱신하고 분량 제한 적용 | Codex | 🟢 DONE | P2 | 2026-07-16 | 2026-07-16 | 100 | OPS-008 | 일일보고 갱신이 같은 주차의 기존 주간보고 갱신으로 이어지고 각 주차 블록이 50줄 이내로 유지됨 | root 지침·상세 절차 연결, 기존 주차 갱신·유사 항목 통합·50줄 초과 시 재요약 규칙 및 현재 1주차 블록 41줄 확인 | `AGENTS.md`, `docs/markdown/daily_reports/README.md`, `docs/markdown/daily_reports/주간보고.md` | 2026-07-16 12:21 | 줄 수는 제목부터 다음 주차 제목 직전까지 빈 줄 포함 계산 |
...
|---|---|---|---|---|---|---|
| 2026-07-20 15:44 | DEPLOY-001 | Codex | 사용자 요청에 따라 로컬 `frontend/` 전체를 Git 추적 대상에서 제외 | DONE 유지 | `git check-ignore frontend/README.md`, staged file 목록 및 원격 push 결과 확인 | `.gitignore`, `docs/markdown/AI_AGENT_WBS.md`, `docs/markdown/daily_reports/minji/일일보고.md`, `docs/markdown/daily_reports/주간보고.md` |
| 2026-07-20 11:50 | DEPLOY-001 | Codex | React+Tremor 프론트 작업용 로컬 디렉터리와 Git·환경변수 운영 안내 구성 | TODO → DONE | 디렉터리 구조, `.gitignore`, `git status`, `git diff --check` 확인 | `frontend/`, `.gitignore`, `docs/markdown/AI_AGENT_WBS.md`, `docs/markdown/daily_reports/minji/일일보고.md`, `docs/markdown/daily_reports/주간보고.md` |
