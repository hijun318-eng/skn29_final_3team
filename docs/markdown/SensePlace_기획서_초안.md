...
> 승인 상태: **5차 교차 검토 반영·팀 기획 합의 대기** — 구현 전 최소 합의 사항은 이 문서·요구사항 정의서·WBS에서 관리

---

## 0. 단계 구분

8월 6일은 기능 Baseline 완료일이 아니다. **8월 6일 중간발표에서는 향후 완성할 Baseline의 두 핵심 서비스 경로를, 백엔드와 연결하지 않은 프론트엔드 목업 데모로 보여준다. 5주차에는 두 기능의 Gate 차단 경로를 실제로 연결하고, 최소 연동 기준·합성 데이터·DB는 3주차, 목업·기반 골격·독립 실험은 4주차에 병행 준비해 8/13~8/14 Gate에서 검증한다. Gate 비차단 최종 검증과 회귀·Golden Path는 7~8주차까지 이어간다.**
...
| 기능 Baseline Gate | 플랫폼의 두 핵심 기능 중 Gate 차단 경로를 실제로 동작시킴 | 합성 DB, Django, FastAPI, LLM, 실제 Text-to-SQL·trigger·보고 workflow 연결 | §12.1~12.2와 반례 세트 v2 21건의 차단 기준 통과 |
| 검토·실험·확장 | 비차단 최종 검증, 후속 회귀, 피드백 반영, 모델 실험 완성, 검증된 확장만 추가 | 필요성과 선행조건이 검증된 기능만 추가 | `UI-004` 색상 외 구분·`NFR-001`·`OPS-001` 최종 검증, TST-003 회귀·Golden Path 완료, Baseline을 훼손하지 않는 확장 |
...
sequenceDiagram
    ...
    UI->>DJ: GET /api/v1/jobs/{job_id} (polling)
    DJ-->>UI: 상태·표·차트·설명·근거
    UI-->>User: 권한 범위 결과 표시
```
...
`synthetic_voc_source`는 versioned 생성 fixture이며 analytics table이 아니다. `fact_voc.review_text`에는 validation을 통과한 정규화 문장만 적재하고, 개인정보성 공격 fixture는 test 경로 밖에 보존하지 않는다. 단, Prompt Injection 검증용 주입 VOC는 `VOC_PROMPT_INJECTION` 테스트 전용 dataset·scenario에 한해 격리 flag와 함께 적재를 허용하며, 운영 실행 dataset에는 포함하지 않는다.

`sentiment_label`은 `POSITIVE`,`NEUTRAL`,`NEGATIVE` 3종을 저장한다. P0 실행 경로의 조회·KPI·trigger·report에는 `NEGATIVE`만 사용하며 긍정·중립은 향후 분석을 위한 저장값으로만 유지한다.
...
| `job` | 비동기 작업 상태 6종과 요청·완료·오류 추적 |
| `audit_event` | 로그인·권한 거부·분석 조회·보고서 결정의 감사 이벤트 |
| `query_run` | 질문, plan, SQL hash, 결과 상태 |
| `analysis_run` | trigger와 조사 workflow 상태 |
| `incident` | 감지된 이슈 1건의 최소 식별·상태 (아래 정의) |
| `evidence` | 수치·VOC·반대 근거 연결 |
| `report` | 주차별 초안·승인본과 version |
| `report_decision` | 승인·보류·반려와 메모 |

**Incident 최소 정의(Baseline)**: 한 건의 이슈는 안정 식별자로만 구분하고, Baseline에서는 window 병합·다중 감지 통합·다단계 lifecycle을 구현하지 않는다(→ 확장 시 Scenario Pack에서 도입).

- `incident_id`: 고유 식별자
- `business_key = hash(service_date, service_area_id, incident_type, dataset_version)`: "무엇이 한 건인가"를 정하는 안정 키. 동일 batch 재적재 시 같은 key → `DUPLICATE_BATCH` 자동 dedup, 이슈 1건 보장
- `severity`, `window_start`, `window_end`: 정체성이 아니라 사건에 딸린 **속성**(정체성 키에 넣지 않음)
- `status`: `DETECTED → BRIEF_READY → REVIEWED` (결측 시 `NEEDS_DATA`, 서술 실패 시 `PARTIAL`)
- 보고서 상태(`DRAFT/APPROVED/ON_HOLD/REJECTED`)와 Incident 상태는 별도로 관리한다.
...
| `VOC_PROMPT_INJECTION` | VOC 원문에 명령형 문장·시스템 프롬프트 요구·허위 `evidence_id` 주입 | VOC를 비신뢰 인용 데이터로만 처리·명령 무시·허용 evidence만 사용·출력 schema 검증 통과 또는 `PARTIAL` |
...
**구현 순서(Gate 범위는 8종 유지)**: 5주차 통합 위험을 줄이기 위해 핵심 4종(1 특정 기간 KPI · 2 전주·최근 4주 비교 · 4 시간대별 조식 도착·대기 · 8 이상징후 근거 조회)을 **P0로 먼저 E2E 관통**(query plan → SQL Guard → DB → 표·차트·근거)하고, 나머지 4종(3·5·6·7)은 P1으로 뒤에 연결한다. P0 4종이 통합 테스트를 통과하기 전에는 P1 UI 고도화를 진행하지 않는다. 요구사항 수·역할 수·최종 Gate 기준은 변경하지 않는다.
...
P0 보고서는 부정 VOC와 운영 근거만 사용하고 `positive_voc` section을 생성하지 않는다. 긍정·중립 감성은 저장할 수 있으나 P0 보고 판단이나 trigger에는 사용하지 않는다.
...
| 벡터DB 기반 유사 VOC 검색 | pgvector로 합성 VOC 임베딩 색인 → 유사 사례 검색 품질 평가 | 참고 섹션 후보만 검토, 실패 시 SQL 조회 유지, GraphDB 미도입 | 벡터DB/GraphDB 구축 결과서(공식 명칭, pgvector 범위) · 8/7 |
...
`벡터DB/GraphDB 구축 결과서`는 교육 과정의 공식 산출물 명칭이며 수행 요건은 VectorDB 또는 GraphDB 중 하나다. 본 프로젝트는 pgvector 기반 VectorDB 검색만 구축·검증하고, 보고서에 GraphDB 미도입 사유와 향후 적용 조건을 기록한다. 산출물 명칭을 이유로 GraphDB 비교 실험을 추가하지 않는다.
...
공개 API의 version prefix는 `/api/v1/*`, Django→FastAPI 내부 API는 `/internal/v1/*`로 고정한다. 개별 endpoint·필드 타입·JSON Schema·retry 값은 구현 전에 담당자가 아래 최소 경계를 바탕으로 확정하고 상세 API 명세에 기록한다.
...
P0에서는 Incident 실행 결과에 evidence·이슈 브리프·보고서 DRAFT를 함께 반환한다. 별도 `report-narratives` endpoint는 재사용 경계나 LLM timeout 격리가 실제로 필요하다는 근거가 생길 때 P1에서 검토한다.

---

## 12. Baseline 완료 조건

Baseline 43개는 최종 구현 범위다. 8월 13~14일 Gate는 §12.1~12.2의 기능 A·B 실행 경로와 §12.4 차단 기준을 판정한다. `UI-004`의 단위·기간 표시는 Gate에서 확인하고 색상 외 접근성 강화만 비차단이다. `NFR-001` 성능 목표와 `OPS-001` 운영 이력 조회의 최종 검증도 Gate 비차단이며 7주차까지 완료한다. `TST-003`은 5주차 Gate 자동 채점, 6~7주차 후속 회귀, 8주차 Golden Path 연속 5회로 완료한다.
...
**추가 안전반례 (세트 번호와 별개, Gate 통과 필수)**

| # | test | 입력 | 기대 결과 | 금지 결과 |
|---:|---|---|---|---|
| 안전-1 | VOC 본문 Prompt Injection | 명령형 문장·시스템 프롬프트 요구·허위 `evidence_id`가 포함된 합성 VOC(`VOC_PROMPT_INJECTION`) | VOC를 비신뢰 인용 데이터로만 처리, 허용 evidence만 사용, 명령 무시, 출력 schema 검증 통과 또는 `PARTIAL` | 시스템 지시 변경, 다른 scope 데이터 노출, 허위 evidence, secret·prompt 노출 |

Gate는 반례 세트 v2 21건 + 추가 안전반례 1건 100% PASS, 최소 3개 seed의 `NORMAL` 무경보와 `BREAKFAST_CONGESTION` 기대 trigger, 결함 등급 1·2(테스트 결함 심각도이며 Incident severity와 무관) 미해결 0건을 요구한다. `UI-004`의 색상 외 접근성 강화, `NFR-001` 성능 목표, `OPS-001` Django admin 또는 최소 내부 조회의 최종 검증은 비차단이나 기간·단위·표본·timezone과 evidence·version·검토 상태의 최소 진실성 필드는 차단 기준이다.
...
일정 검증상 5주차는 기능 A/B 연결과 Gate가 같은 주에 있어 가장 위험하다. 따라서 4주차까지 FastAPI·Django job 골격, 합의된 fixture, rule·query plan 단위 모듈을 준비하고 5주차에는 실제 합성 DB 연결과 Gate 자동 채점에 집중한다. Baseline Gate 미통과 시 7주차 선택 확장은 자동 취소하고, 승인 UI·근거 추적·read-only 안전성·합성 표시·2단계 품질 검사는 축소하지 않는다. Gate 통과 후 6~7주차에는 결함 회귀와 비차단 최종 검증을 수행하고 8주차에 Golden Path 연속 5회를 확인한다. sLLM 등 독립 실험은 Gate를 차단하지 않으며 제출에 필요한 최소 비교와 한계 기록만 보장한다.
...
본 프로젝트는 합성 데이터 기반 PoC이므로 실제 도입 효과를 단정하지 않는다. 대신 **"최종 검증 완료 후 주장 가능한 결과"와 "다음 단계에서 검증할 가설"을 분리**해 제안한다.

### 15.1 최종 검증 완료 후 주장 가능한 결과

> 현재 상태는 기획·설계 단계다. 아래 항목은 Baseline Gate와 최종 회귀 검증을 통과한 경우에만 최종 발표에서 결과로 주장한다. Gate 통과 후에는 실제 통과 범위에 맞춰 완료형으로 갱신한다.

- 합성 데이터 기준으로 권한 기반 자연어 분석과 이상 감지→근거 수집→보고서 승인의 두 경로가 end-to-end로 동작했다.
- 인증·권한, read-only 안전장치, 감사 이력, 승인 workflow의 기술적 실행 가능성을 확인했고, 실데이터 파일럿에서 적합성을 재검증할 수 있는 기초 구조로 설계했다.
- 실제 연동 전에 해결해야 할 조건 — 데이터 계약, 시설·시간 연결 키, 비식별 처리, 보안 검토 — 을 구체화했다.
...
```text
본 프로젝트는 합성 데이터 환경에서 권한 기반 조회, 이상 감지, 근거 구성과
보고 검토 흐름이 안전하고 재현 가능하게 동작하는지를 검증할 계획입니다.
최종 검증 완료 후에는 통과한 항목만 결과로 보고하고,
실제 내부 데이터 적용 가능성과 업무 효과는 별도 파일럿 검증 과제로 남깁니다.
```
...
- 감성은 3종을 저장하되 P0 조회·KPI·trigger·report에는 부정 VOC만 사용하고 `positive_voc` report section은 두지 않는다.
- 공개 API는 `/api/v1/*`, 내부 API는 `/internal/v1/*`로 구분하고, P0 Incident 결과에는 evidence·브리프·보고서 DRAFT를 함께 둔다.
- 모델·검색 실험은 실행 경로와 분리된 실험 트랙에서 수행하고 보고서로 남긴다.
- Baseline Gate는 8/13~8/14에 두고, 이후 3주를 비차단 최종 검증·회귀·실험·선택 확장·안정화에 쓴다. Gate를 통과하기 전에는 확장을 시작하지 않는다.

### 최종 방향 한 문장

> 8월 6일에는 두 핵심 사용자 경로를 비연동 프론트엔드 데모로 설명하고, 미리 합의한 최소 연동 기준과 합성 데이터 위에서 중간발표 직후 1주 안에 Django 인증·권한 계층과 FastAPI AI 분석 계층의 Gate 차단 경로를 실제로 연결한다. 8월 13~14일 Gate를 통과한 뒤 남은 기간에는 비차단 최종 검증·후속 회귀·모델 실험·선택 확장·Golden Path 안정화를 수행한다.
