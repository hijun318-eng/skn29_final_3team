...
Gate는 반례 세트 v2 21건 100% PASS, 최소 3개 seed의 `NORMAL` 무경보와 `BREAKFAST_CONGESTION` 기대 trigger, severity 1·2 미해결 결함 0건을 요구한다. `UI-004`의 색상 외 접근성 강화, `NFR-001` 성능 목표, `OPS-001` Django admin 또는 최소 내부 조회의 최종 검증은 비차단이나 기간·단위·표본·timezone과 evidence·version·검토 상태의 최소 진실성 필드는 차단 기준이다.
