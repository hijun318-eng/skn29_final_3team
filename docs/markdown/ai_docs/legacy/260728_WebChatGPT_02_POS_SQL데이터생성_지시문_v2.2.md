# 웹 ChatGPT 02
## F&B POS·좌석 운영 SQL 데이터 적재 지시문 v2.2
...
이 ChatGPT는 **`hotel_pos` MySQL 데이터 적재만** 담당한다.
...
- 논리 `store`, `order`, `order_line`은 각각 `pos_stores`, `pos_orders`, `pos_order_items`다.
- 논리 `item`은 `pos_order_items.item_code`·`item_category`로 구현한다.
- 논리 `payment`은 현재 P0/P1에서 `pos_orders.payment_status`·`payment_amount`로 구현한다.
- 다중 결제수단이나 상품 master가 승인되기 전에는 별도 `item`·`payment` 테이블을 임의 추가하지 않는다.
- `pos_customer_ref`는 같은 문자열의 다른 source ID와 직접 JOIN하지 않고 event time에 유효한 CRM identity mapping만 사용한다.

### 생성할 SQL 파일

```text
260728_02_pos_mysql_2022_2026_v2.2.sql
```
...
`pos_generation_audit`는 생성하거나 사용하지 않는다.
...
날짜 hash 입력은 `DATE_FORMAT(value, '%Y-%m-%d')` 또는 ISO datetime으로 명시 변환한다.
...
기존 컬럼에 `property_id`를 추가하고 다음 UNIQUE를 사용한다.

```text
UNIQUE(property_id, store_id)
```

### `pos_service_periods`

기존 컬럼에 `property_id`를 추가한다.

```text
UNIQUE(property_id, store_id, business_date, service_period)
```

- 실제 covers와 seat_hours_used는 `simulation_as_of_date`까지만 생성한다.
- 미래 날짜의 service period 행은 생성하지 않는다.
- 미래 좌석 계획은 별도 Scenario View가 담당한다.
...
```text
ordered_at <= check_closed_at <= source_updated_at
ordered_at <= generation_as_of_at
```

2026-07-29 이후의 미래 주문을 생성하지 않는다.

### `pos_order_items`

기존 컬럼에 `property_id`를 추가한다.
부모 주문과 같은 `property_id`를 가져야 한다.
...
```text
pos_stores          8건
pos_service_periods 45,000~67,000건
pos_orders          280,000~500,000건
pos_order_items     750,000~1,450,000건
```
...
다만 PMS 행을 직접 조회하지 않고 공통 날짜·계절·고객키 규칙만 사용한다.
...
- 주문 할인과 품목 할인을 중복 차감하지 않도록 명시한다.
- `VOID`: gross는 원거래 기록 가능, net=0, payment=0, refund=0, void_flag=true
- `REFUNDED`: refund_amount = 결제 전 순액, net=0, payment_amount=0
- `PARTIAL_REFUND`: 0 < refund_amount < 결제 전 순액
- `PAID`: refund_amount=0, payment_amount=net_amount
- `PARTIAL_REFUND`: payment_amount=net_amount이며 결제 후 남은 순액을 뜻한다.
- `REFUNDED`: payment_status=REFUNDED, `payment_amount=0`
- `VOID`: payment_status=FAILED, `payment_amount=0`
- 모든 환불은 별도 `refund_amount`로 표현한다.
- 미래 주문을 forecast 거래로 생성하지 않는다.

### 안정 ID

```text
order_id:
property_id + store_id + business_date + service_period + order_slot

order_item_id:
property_id + order_id + item_sequence
```

주문 수가 일부 바뀌어도 다른 날짜·매장의 ID와 가격 noise가 재추첨되지 않아야 한다.

### 고객키

- CRM 연결 주문은 `POSC-` 8자리 형식
- 연결 비율 55~70%
- 고객키는 주문 발생일보다 늦게 생성된 회원을 전제하지 않도록 CRM 계약과 숫자 범위를 맞춘다.
- Source 간 물리 FK는 금지한다.
- `pos_order_items`의 기간 상태는 반드시 부모 `pos_orders`를 통해 판정한다.
...
- MySQL 8.0 이상
- `INSERT ... SELECT` 집합 생성
- PK·UNIQUE ID에는 `SHA2(...,256)` 또는 충돌 없는 자연키 문자열을 사용
- `CRC32(CONCAT(...))`는 bucket·noise 생성에만 사용하며 PK·UNIQUE ID 생성에는 사용 금지
- `RAND()` 사용 금지
- FK 자식→부모 순서로 대상 합성 행 삭제
- DDL·index·temporary permanent table 생성 금지
- 임시 CTE 또는 TEMPORARY tally table은 허용하되 종료 시 정리
- transaction 사용
- `DROP TABLE` 금지
...
quantity <= 0 건수 0
ordered_at > source_updated_at 0
check_closed_at > source_updated_at 0
2026-07-29 이후 order 0
gross < discount+refund 건수 0
품목 gross 합계와 order gross 불일치 0
품목 할인+주문 할인 이중 차감 0
net amount 공식 불일치 0
VOID인데 payment_amount > 0 건수 0
REFUNDED인데 net_amount > 0 건수 0
PAID인데 refund_amount > 0 건수 0
PARTIAL_REFUND인데 payment_amount <> net_amount 건수 0
VOID인데 payment_status <> FAILED 건수 0
order_item을 단독 기간 fact로 집계한 쿼리 0
seat_hours_used > seat_hours_available 비정상 건수 0
2026-07-29 이후 is_forecast=false fact 0
실제 개인정보 패턴 0
```

요약 SELECT는 실제·YTD만 출력하며 forecast 거래량은 0건임을 별도 확인한다.
...
좌석시간 이용률
환불·무효율
매장·service period별 월간 추세
table별 row count·watermark·checksum
```
...
SQL 파일 하나를 생성하고 링크와 검증 요약만 제공한다.
