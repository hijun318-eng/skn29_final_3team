...
5. 병합된 `dev`에서 `docs/markdown/daily_reports/README.md`의 `dev 병합 요청 시 보고 통합` 절차로 보고를 생성·검증한다.
6. 위 절차가 반환한 `team_summaries/` 변경 파일만 stage해 staged diff를 확인하고, 변경이 있으면 보고 전용 commit 하나를 만든다. 변경이 없으면 생략한다.
7. 병합 결과와 보고 전용 commit을 함께 `origin/dev`에 push한다.
