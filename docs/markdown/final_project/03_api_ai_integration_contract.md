...
Baseline에서 React는 Django의 5개 통합 demo endpoint만 호출한다. Django가 역할 검사·local persistence·업무 상태를 담당하고 `src`를 직접 호출하며, FastAPI는 실행하지 않는다. 아래 8개 공개 API와 3개 내부 AI 계약은 P0 강화 목표이고 첫 프로토타입의 물리 endpoint 수가 아니다.
...
- Django 통합 demo endpoint 5개로 `API-001`~`API-008`의 용도를 묶어 제공
- Django가 `src/analysis`의 typed Python interface를 직접 호출
- AI는 선택 사항이며 `build_signal_explanation(analysis_snapshot)` 계약 1개만 사용
- AI 미사용·실패 시에도 rule·metric·evidence·template report 유지
- V1·V2 전환은 management command 1개만 사용

| Baseline method·path | 포괄하는 P0 API | 용도 |
|---|---|---|
| `GET /api/v1/demo/summary` | `API-001`, `API-002` | KPI와 signal 목록 |
| `GET /api/v1/demo/signals/{id}` | `API-003`, `API-004` | signal 상세와 evidence |
| `POST /api/v1/demo/signals/{id}/field-note` | `API-005` | 현장 메모 저장 |
| `GET /api/v1/demo/report` | `API-006`, `API-007` | 단일 주간 report 작업본 |
| `PATCH /api/v1/demo/report/decision` | `API-008` | 승인·보류·반려 |
...
Baseline은 실제 계정·SSO 없이 두 demo 역할을 요청 context에 넣고 Django가 허용 작업을 검사한다. 아래 객체 단위 인증·권한 계약은 외부 배포 또는 P0 강화 시 적용한다.
...
아래 세분 endpoint는 Baseline E2E 통과 후 목록 분리·pagination·객체 권한·version 충돌 처리가 필요할 때 적용한다.
...
Baseline은 세 계약을 각각 endpoint로 만들지 않는다. 하나의 typed Python interface가 필요한 설명·보고 문장을 생성하며, 분류는 fixture label 또는 결정론적 mapping으로 처리할 수 있다. 분리 배포할 경우에만 아래 schema를 내부 API로 적용한다.
...
| `1.1` | 2026-07-20 | Baseline을 Django 통합 endpoint 5개와 typed Python interface 1개로 축소하고 세분 API를 P0 강화로 이동 |
