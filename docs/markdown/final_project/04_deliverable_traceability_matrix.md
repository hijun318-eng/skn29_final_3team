...
요구사항·산출물·데이터·API·화면·모델·test·evidence를 `DOC-*` 기준으로 연결한다. 파일이 존재하는 것과 구현·검증이 완료된 것은 다르다. 현재 구현 code와 실행 evidence가 없으므로 문서 초안만 있는 산출물도 `완료`로 표시하지 않는다.
...
P0 구현 추적은 `REQ-F-001`~`REQ-F-007`, `REQ-NF-001`~`REQ-NF-002`, `DATA-001`~`DATA-008`, `API-001`~`API-008`, 화면 `P0-*`, `RULE-001`, `TC-*`로 연결한다. 실제 code가 생기기 전까지 `source_code_paths`는 구현 경계와 `미구현`을 함께 표시한다.
