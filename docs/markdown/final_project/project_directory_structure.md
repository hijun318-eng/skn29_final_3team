...
이 문서는 Hotel Signal AI 최소 MVP의 파일 배치와 서비스 책임을 정한다. 실제 코드가 없는 폴더를 먼저 증식시키는 지침이 아니라, 구현이 시작될 때 파일을 어디에 둘지 결정하는 기준이다.
...
FastAPI는 사용자 DB, 권한 DB, migration, 원시 SQL 실행 API를 가지지 않는다. 실제 분석 서비스를 분리할 필요가 확인되기 전에는 폴더를 만들지 않고 Django가 `src`를 직접 호출하는 단일 프로세스도 허용한다.
...
| 기능 | 위치 | 조건 |
|---|---|---|
| 화면·상태 표현 | `app/react/` | 승인된 P0 화면 계약 존재 |
| 권한·업무 API·DB | `app/django/` | 데이터 소유권과 API 계약 확정 |
