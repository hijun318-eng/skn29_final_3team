...
|---|---|
| 기준일 | 2026-08-22 KST |
| 제품 단계 | DataHub-first 전환 Phase 9 격리 Gate 통과, Phase 10 P0 same-release 봉인 `BLOCKED` |
...
- DataHub environment, governed context release/checksum, Term·Asset·Document/native semantic URN/aspect
- immutable RuntimeCatalogProjection, permission policy/prompt/model 버전과 Docker image digest
- 테스트 명령·시각·환경·결과 원문
...
| 데이터 거버넌스 담당자 | DataHub에서 Glossary, Dataset/schema, lineage, Context Documents, Structured Properties와 지원된 native semantic을 관리 | 애플리케이션 내부 semantic authoring UI |
...
|---|---|
| `ANALYSIS` | Metric·기간·filter·dimension·grain·비교 대상 중 하나라도 데이터 의미를 바꾸는 **완전하게 resolved된 요청**은 새 `analysis_run_id`를 만들고 현재 권한·DataHub release·RuntimeCatalogProjection으로 APP-G1→G2→Trino→G3를 다시 통과한다. preflight에서 막힌 Analysis Turn은 Run이 0건이다. |
...
| DATA-002 | 읽기 전용 check의 predecessor·target catalog hash를 명시적 publish가 다시 요구하며, publish 전후 DataHub governed entity/aspect를 전체 read-back 한다. 부분 publish는 active release가 되지 않는다. | `READY_TO_VERIFY` |
| DATA-003 | release compiler는 `scrollAcrossEntities` 전체 멤버십, entity/aspect exact read-back과 Trino `information_schema` fingerprint가 일치할 때만 immutable RuntimeCatalogProjection을 만든다. 질문 runtime의 bounded Search와 full release read-back을 분리하고 특정 view 수나 FQN 목록은 코드에 두지 않는다. | `READY_TO_VERIFY` |
| DATA-004 | 질문 retrieval은 bounded DataHub lexical Search에서 URN/type 후보만 얻고 App entitlement와 active release membership을 적용한 뒤 최소 projection만 읽는다. `lexical_shadow`의 품질·negative closure·p95·freshness·failure를 사전 threshold와 비교해 `PROMOTE`·`HOLD`·`REJECT`를 기록하며, promote일 때만 canary·default 전환·rollback을 순서대로 검증한다. semantic/hybrid는 별도 Gate S1 전에는 기본 경로가 아니다. | `PARTIAL` |
...
| GOV-003 | Node 1 전에 bounded DataHub Search로 URN/type 후보를 발견하고 entitlement·active release membership을 적용한다. Node 1은 최소 Node1InterpretationContext만 받고, App이 candidate reference를 다시 결속한 뒤 RuntimeContextPackage를 만든다. | `PARTIAL` |
| GOV-004 | 런타임은 삭제·미승인 Term, 관계 단절, schema drift, alias 충돌, unit 불일치를 APP-G1 전에 차단한다. | `PARTIAL` |
| GOV-005 | 새 semantic authoring의 권위는 DataHub Governed Context Plane에 둔다. 실행 계산·filter·join·time·identity는 stable URN/aspect에서 immutable RuntimeCatalogProjection으로 compile하고, entitlement·permission snapshot은 App이 소유한다. 기존 Rule/Semantic Registry는 equality가 확인될 때까지 이행 입력으로만 사용하며 독립 authoring SoT로 연장하지 않는다. | `PARTIAL` |
| GOV-006 | 로컬 JSON은 공개 schema·version manifest·설정·test fixture로만 사용한다. 현재 full loader는 Phase 4 projection equality 전까지 compiler/read-back 경계로 유지하고, refresh 실패나 출처 불일치는 `METADATA_UNAVAILABLE`로 fail-closed 한다. | `READY_TO_VERIFY` |
| GOV-007 | Context·trace·결과 근거에 DataHub Term·Dataset/Column/Document/native semantic URN과 aspect checksum, RuntimeCatalogProjection checksum, product release와 permission snapshot을 보존하고 화면에서 확인할 수 있다. | `PARTIAL` |
| GOV-008 | **Candidate readiness:** DataHub governed entity/aspect를 stable identity의 staging/shadow 영역에 out-of-place publish하고 full Scroll·exact entity/aspect read-back·Trino fingerprint로 immutable RuntimeCatalogProjection을 compile한다. projection equality와 명시적 `product_release_id` canary 전에는 runtime 기본 경로에 노출하지 않는다. DataHub mutation은 RBAC로 제한하고 사용자별 entitlement는 매 Run의 permission snapshot으로 고정한다. | `NOT_STARTED` |
| GOV-009 | **Activation:** pre-cutover Gate 승인 뒤 data·DataHub governed context·RuntimeCatalogProjection·App 기본값을 같은 `product_release_id`로 바꾼다. readiness·Golden smoke 실패 시 이전 projection/pointer와 cache namespace를 복원하고 부분 publish나 서로 다른 릴리스 조합이 보이지 않음을 L4 receipt로 증명한다. | `NOT_STARTED` |

현재 host source는 DataHub full membership, bounded/semantic Search, Domain·Owner·Lifecycle, Term/Column 관계와 structured properties를 읽는 경로를 포함하지만 일부는 deployed image와 다르다. live active release는 51 asset·10 required Term·14 compiled metric이고 native Metric은 0이며, 별도 `aiContext` aspect capability와 semantic/hybrid는 검증되지 않았다. host 구현을 DataHub-first authority나 production E2E 완료로 발표하지 않는다.
...
| AUTH-002 | 모든 P0 API에서 역할과 객체 소유권을 서버가 검사하며 URL·body 변조로 타인의 Conversation·Turn·Analysis·Artifact·View·Report에 접근할 수 없다. Conversation·Turn API와 path 소유권 검사는 있으나 path `conversation_id`의 RequestContext 결속과 전체 객체 ID 주입 negative test는 완료되지 않았다. | `PARTIAL` |
| AUTH-003 | DataHub Search 후보·RuntimeCatalogProjection, Trino 조회, 결과 열·행에 같은 권한 스냅샷을 적용하고 trace에 policy version을 기록한다. | `PARTIAL` |
...
| ANL-003 | 확인 응답은 원 요청과 연결된 새 실행을 만들며 선택하지 않은 해석을 trace에서 제외한다. 현재 Frontend는 Conversation command를 사용하지만 clarification의 원 Turn 결속, no-Run 보장과 동일 idempotency key retry는 같은 release E2E로 검증되지 않았다. | `PARTIAL` |
| ANL-004 | RuntimeContextPackage에는 DataHub Term·Asset·Field/aspect, RuntimeCatalogProjection rule, absolute time range, Backend-owned `wall_clock_anchor`, 별도 data availability watermark, permission snapshot, product version이 포함되고 APP-G1을 통과한다. wall clock과 data cutoff를 같은 `as_of` 값으로 혼용하지 않는다. | `PARTIAL` |
...
| `CONV-005` | 서버는 Turn을 `ANALYSIS`·`PRESENTATION`·`REPORT_ACTION` 중 하나로 typed routing한다. 명확화는 원 route의 `BLOCKED + NEEDS_CLARIFICATION` 결과다. 분류 model이 있더라도 권한, Artifact schema, 허용 chart, query 필요 여부와 Report 대상은 결정론적 validator가 최종 판정한다. 현재 host에는 세 route 분기 코드가 있으나 required idempotency/CAS, terminal status, release/permission lineage와 Golden Dialogue를 통과하지 않아 이 Requirement를 시작된 제품 경로로 승인하지 않는다. | `NOT_STARTED` |
...
| `CONV-008` | Conversation 생성 시 서버가 하나의 `product_release_id`와 RuntimeCatalogProjection checksum을 고정하고 모든 Turn·Analysis Run·Artifact·presentation·Report lineage가 이를 상속한다. active pointer 변경으로 기존 대화를 다른 data·projection·policy 조합에 조용히 재해석하지 않으며, 고정 릴리스가 실행 불가하면 `409 RESOURCE_CONFLICT`로 새 Conversation 전환을 안내한다. 현재 host는 active product/semantic release와 permission snapshot을 Conversation에 pin하고 ANALYSIS Turn·Run·Artifact에 전파하는 격리 Gate를 통과했다. RuntimeCatalogProjection checksum, View·Report 전체 전파, 실제 배포·browser 증거는 Phase 4·7·10에 남아 있다. | `PARTIAL` |
| `CONV-009` | mutation은 먼저 `(owner, conversation_id, idempotency_key)`와 저장 canonical hash를 비교한다. 같은 key·같은 hash면 head가 이미 전진했어도 기존 command/Turn을 반환하고, 같은 key·다른 hash면 `409 IDEMPOTENCY_CONFLICT`다. 새 key일 때만 required `expected_head_turn_id` CAS와 active lease를 검사해 command를 예약한다. 서버 hash는 path conversation, subject, permission, release와 확정 route를 포함한다. 현재 host는 key·명시적 head를 필수화하고 hash-before-replay, path/subject/permission/release 결속, Frontend 동일 key retry를 unit·격리 integration에서 검증했다. 확정 route의 admission-time hash와 actual browser concurrency/reconnect 증거가 남아 있어 `VERIFIED`로 올리지 않는다. | `PARTIAL` |
...
| SAVE-002 | 저장 분석 재실행은 현재 권한·DataHub release·RuntimeCatalogProjection을 다시 검증하고 새 Run·query ID를 만든다. 과거 결과를 현재 결과처럼 복사하지 않는다. | `READY_TO_VERIFY` |
...
| ID | 요구사항과 인수 조건 | 상태 |
|---|---|---|
| OPS-001 | readiness가 App DB, DataHub governed context, RuntimeCatalogProjection, Trino source, model endpoint를 각각 실제 probe하고 code/image·data·projection·prompt/model/policy·migration의 `product_release_id` 일치를 검사하며 불일치 시 fail-closed 한다. | `PARTIAL` |
...
| `P0-DATAHUB-SEARCH` | DATA-004, GOV-003~004, GOV-006, OPS-001~002, QA-001~002 |
...
## 7. 후속 Phase와 P1·P2 요구

DataHub-first 전환은 Phase 0B compatibility/evidence → Phase 1 Conversation Safety → Phase 2
DataHub lexical Search → Phase 3 native Metric/AI Context capability → Phase 4 immutable
RuntimeCatalogProjection → Phase 5 Node 1 grounding → Phase 6 same-asset single-turn → Phase 7
bounded multi-turn → Phase 8 나머지 native semantic shadow → Phase 9 multi-asset JOIN → Phase 10
P0 same-release 봉인 순서다. 각 Gate 승인 없이 다음 단계로 넘어가지 않는다.

P1/P2 capability 확장은 Phase 10 뒤 Phase 11 `analysis.run`, Phase 12 `rag.answer`, Phase 13
`ml.predict`, Phase 14 General Orchestrator 순서로 진행한다. `semantic.resolve`는 외부 Tool이 아니라
내부 Context Gateway operation이며, `artifact.get`, `report.add_block`은 필요성이 입증될 때만
조건부다. semantic/hybrid Gate S1과 Neo4j/`graph.resolve`도 정량 gap Gate 없이는 기본 순서에
들어오지 않는다.

예약 실행·worker가 필요한지는 예약 동시성·실행 시간·다중 인스턴스 요구를 실측한 뒤
결정한다. P0에는 새 분산 시스템을 추가하지 않는다.

## 8. 현재 판정의 한계

- Phase 0B~9는 분리된 `answervice-phase2b-datahub` DataHub/App DB와 기존 Trino의 읽기 전용
  실제 조회를 사용해 단계별 Gate를 통과했다. Phase 6~9는 실제 SQL을 실행했고 필요한 범위의
  외부 Node 1/3 모델 호출도 수행했지만, 서로 다른 단계 receipt를 Phase 10 증거로 합치지 않는다.
- 2026-08-23 Phase 10 판정에서 active generation 21의 Phase 9 receipt는 model manifest만 현재
  tree와 일치했다. 현재 source dirty patch와 migration chain은 active receipt와 달랐고,
  image 목록에는 DataHub GMS·PostgreSQL·Trino만 있어 현재 source로 만든 Backend·Frontend
  candidate image가 없었다.
- Phase 10 시점의 격리 DataHub full read-back과 Trino 전체 fingerprint는 active projection과
  일치했고 acceptance principal의 nonterminal query도 0건이었다. 그러나 실제 Browser L3/L4와
  같은 실행에서 생성한 Backend·Frontend image 및 host-validation receipt는 없다.
- 기존 `answervice` stack은 계속 read-only 관측 대상으로 유지했고 migration·pointer·index·container를
  변경하지 않았다. 격리 Gate 성공을 현재 배포 stack의 same-release E2E로 발표하지 않는다.
- 따라서 66개 P0 Requirement와 11개 Release Gate 중 `VERIFIED`는 0개이며, 이 문서의 상태는
  구현 진행도와 fail-closed 인수 판정이지 제품 인증서가 아니다.
