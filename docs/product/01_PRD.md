...
| `admin` | `analyst` 권한 전체, Report 승인·실행, 데이터 연결 상태, 계정·감사·시스템 관리 | DataHub publish, Trino setup, Source DB service identity와 사용자별 객체 권한 우회 |

사람에게 부여하는 App Role은 위 두 개뿐이다. 두 Role 모두 승인 범위의 사업 질문을
실행하고 사업책임자는 검증된 결과를 소비한다. 관리 기능은 사용자명 분기가 아니라
Role→Capability 정책으로 분리한다. 사람 Role 단순화는 보고서 재실행의 effective
subject나 App·Trino·DataHub·Source DB의 최소권한 service identity를 바꾸지 않는다.
로컬 제품 Frontend는 `http://localhost:5173` 한 origin을 사용하고 관리 화면은 같은
애플리케이션의 `/admin` route다. 별도 관리자 Frontend·별도 cookie·별도 로그인 Role을
제품 계약으로 두지 않으며 `analyst`의 route/API 직접 접근은 서버에서도 403으로 막는다.
