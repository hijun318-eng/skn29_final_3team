"""동일 RAG corpus에서 OpenAI와 Qwen embedding 검색 품질을 비교한다."""
...
def normalized(vectors: np.ndarray) -> np.ndarray:
    """입력 벡터를 cosine 유사도 계산용 단위 벡터로 정규화한다."""
...
def ranked_documents(scores: np.ndarray, titles: list[str], limit: int = 5) -> list[str]:
    """유사도 순으로 중복 제목을 제거한 상위 문서 목록을 반환한다."""
...
def openai_queries() -> tuple[np.ndarray, float]:
    """명시된 OpenAI endpoint에서 평가 질문 embedding과 지연 시간을 얻는다."""
...
def main() -> None:
    """동일 corpus 평가를 실행하고 재현 가능한 JSON 보고서를 기록한다."""
