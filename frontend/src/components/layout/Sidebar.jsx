...
export function Sidebar({ collapsed, onToggle }) {
  ...
  return <aside className={`sidebar ${collapsed ? "sidebar--collapsed" : ""}`}>
    <div className="brand"><span className="brand__mark" aria-label="Sense Place"><span className="brand__monogram">SP</span><i aria-hidden="true" /></span>{!collapsed && <span><b>SENSE PLACE</b><small>VOC Intelligence</small></span>}</div>
    ...
    <div className="sidebar__footer">{!collapsed && <div className="manager-profile"><span>K</span><div><b>김지민 · 총괄 매니저</b><small>CX Operations</small></div></div>}<button className="collapse-button" onClick={onToggle} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}>{collapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}</button></div>
  </aside>;
}
