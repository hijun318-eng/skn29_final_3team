  ...
  return (
    <section className="impact-comparison" aria-labelledby="impact-title">
      ...
      <div className="comparison-table" role="table" aria-label="대응 적용 전후 예상 효과">
        ...
        <div className="comparison-row" role="row"><b>혼잡도</b><span className="status-text status-text--danger">위험</span><strong className={`status-text status-text--${result.status === "주의" ? "warning" : "danger"}`}>{result.status}</strong></div>
