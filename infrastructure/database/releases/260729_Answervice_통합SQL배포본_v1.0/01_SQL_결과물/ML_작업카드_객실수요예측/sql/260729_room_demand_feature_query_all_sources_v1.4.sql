...
-- read_only=true / training_data_persistence=false
...
pos_features AS (
    SELECT g.property_id,g.target_date,g.room_type_code,g.horizon_days,
           CAST(sum(CASE WHEN o.ordered_at >= g.prediction_cutoff_at_utc - INTERVAL '7' DAY
                              AND o.ordered_at < g.prediction_cutoff_at_utc
                              AND o.source_updated_at < g.prediction_cutoff_at_utc
                              AND o.is_forecast = 0 THEN o.guest_count ELSE 0 END) AS double) AS fnb_covers_lag_7d,
           CAST(sum(CASE WHEN o.ordered_at >= g.prediction_cutoff_at_utc - INTERVAL '7' DAY
                              AND o.ordered_at < g.prediction_cutoff_at_utc
                              AND o.source_updated_at < g.prediction_cutoff_at_utc
                              AND o.is_forecast = 0 THEN o.net_amount ELSE 0 END) AS double) AS fnb_net_amount_lag_7d,
           1 AS pos_source_available
    FROM target_grid g
    LEFT JOIN pos.hotel_pos.pos_orders o ON o.property_id=g.property_id
    GROUP BY 1,2,3,4
),
crm_features AS (
    SELECT g.property_id,g.target_date,g.room_type_code,g.horizon_days,
           CAST(count(DISTINCT CASE WHEN m.member_no IS NOT NULL THEN r.reservation_id END) AS double)
             / NULLIF(CAST(count(DISTINCT r.reservation_id) AS double),0.0) AS member_booking_ratio,
           CAST(count(DISTINCT CASE WHEN h.grade_code='VIP' THEN r.reservation_id END) AS double)
             / NULLIF(CAST(count(DISTINCT r.reservation_id) AS double),0.0) AS vip_booking_ratio,
           1 AS crm_source_available
    FROM target_grid g
    LEFT JOIN pms.public.pms_reservations r
      ON r.property_id=g.property_id AND r.room_type_code=g.room_type_code
     AND r.booked_at<g.prediction_cutoff_at_utc
     AND r.source_updated_at<g.prediction_cutoff_at_utc
     AND (r.cancelled_at IS NULL OR r.cancelled_at>=g.prediction_cutoff_at_utc)
     AND r.checkin_date<=g.target_date AND g.target_date<r.checkout_date
    LEFT JOIN crm.dbo.crm_customer_map m
      ON m.property_id=r.property_id AND m.pms_guest_id=r.guest_id
     AND m.valid_from<CAST(g.prediction_cutoff_at_utc AS timestamp)
     AND (m.valid_to IS NULL OR CAST(g.prediction_cutoff_at_utc AS timestamp)<m.valid_to)
     AND m.source_updated_at<CAST(g.prediction_cutoff_at_utc AS timestamp)
    LEFT JOIN crm.dbo.crm_member_grade_history h
      ON h.property_id=m.property_id AND h.member_no=m.member_no
     AND h.valid_from<CAST(g.prediction_cutoff_at_utc AS timestamp)
     AND (h.valid_to IS NULL OR CAST(g.prediction_cutoff_at_utc AS timestamp)<h.valid_to)
     AND h.source_updated_at<CAST(g.prediction_cutoff_at_utc AS timestamp)
    GROUP BY 1,2,3,4
),
facility_features AS (
    SELECT g.property_id,g.target_date,g.room_type_code,g.horizon_days,
           CAST(sum(CASE WHEN e.event_type='INCIDENT'
                              AND e.event_at>=g.prediction_cutoff_at_utc-INTERVAL '7' DAY
                              AND e.event_at<g.prediction_cutoff_at_utc
                              AND e.source_updated_at<g.prediction_cutoff_at_utc
                         THEN e.downtime_minutes ELSE 0 END) AS double) AS facility_downtime_lag_7d,
           1 AS facility_source_available
    FROM target_grid g
    LEFT JOIN facility.hotel_facility.facility_events e ON e.property_id=g.property_id
    GROUP BY 1,2,3,4
),
banquet_features AS (
    SELECT g.property_id,g.target_date,g.room_type_code,g.horizon_days,
           CAST(count(DISTINCT CASE WHEN b.confirmed_at<g.prediction_cutoff_at_utc
                                         AND b.source_updated_at<g.prediction_cutoff_at_utc
                                         AND (b.cancelled_at IS NULL OR b.cancelled_at>=g.prediction_cutoff_at_utc)
                                         AND b.group_checkin_date<=g.target_date
                                         AND g.target_date<b.group_checkout_date
                                    THEN b.banquet_event_id END) AS double) AS confirmed_banquet_count,
           CAST(sum(CASE WHEN b.confirmed_at<g.prediction_cutoff_at_utc
                              AND b.source_updated_at<g.prediction_cutoff_at_utc
                              AND (b.cancelled_at IS NULL OR b.cancelled_at>=g.prediction_cutoff_at_utc)
                              AND b.group_checkin_date<=g.target_date
                              AND g.target_date<b.group_checkout_date
                         THEN b.expected_guests ELSE 0 END) AS double) AS confirmed_banquet_expected_guests,
           CAST(sum(CASE WHEN b.confirmed_at<g.prediction_cutoff_at_utc
                              AND b.source_updated_at<g.prediction_cutoff_at_utc
                              AND (b.cancelled_at IS NULL OR b.cancelled_at>=g.prediction_cutoff_at_utc)
                              AND b.group_checkin_date<=g.target_date
                              AND g.target_date<b.group_checkout_date
                         THEN b.reserved_room_block_count ELSE 0 END) AS double) AS confirmed_room_block_count,
           CAST(sum(CASE WHEN b.confirmed_at<g.prediction_cutoff_at_utc
                              AND b.source_updated_at<g.prediction_cutoff_at_utc
                              AND (b.cancelled_at IS NULL OR b.cancelled_at>=g.prediction_cutoff_at_utc)
                              AND b.group_checkin_date<=g.target_date
                              AND g.target_date<b.group_checkout_date
                         THEN b.expected_room_nights ELSE 0 END) AS double) AS confirmed_expected_room_nights,
           1 AS banquet_source_available
    FROM target_grid g
    LEFT JOIN banquet.public.banquet_bookings b ON b.property_id=g.property_id
    GROUP BY 1,2,3,4
),
labels AS (
    SELECT
