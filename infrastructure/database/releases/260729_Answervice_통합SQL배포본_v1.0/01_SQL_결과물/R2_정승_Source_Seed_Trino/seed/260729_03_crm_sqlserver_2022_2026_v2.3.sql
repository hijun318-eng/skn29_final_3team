...
-- output=260729_03_crm_sqlserver_2022_2026_v2.3.sql
...
SELECT 'current_grade_duplicate' check_name,COUNT(*) violation_count FROM (SELECT member_no FROM dbo.crm_member_grade_history WHERE property_id='SYNTHETIC_HOTEL_001' AND valid_from<'2026-07-28T05:00:00' AND (valid_to IS NULL OR '2026-07-28T05:00:00'<valid_to) GROUP BY member_no HAVING COUNT(*)>1)x;
SELECT 'current_grade_snapshot_mismatch' check_name,COUNT(*) violation_count FROM dbo.crm_members m JOIN dbo.crm_member_grade_history h ON h.property_id=m.property_id AND h.member_no=m.member_no AND h.valid_from<'2026-07-28T05:00:00' AND (h.valid_to IS NULL OR '2026-07-28T05:00:00'<h.valid_to) WHERE m.property_id='SYNTHETIC_HOTEL_001' AND m.membership_grade<>h.grade_code;
