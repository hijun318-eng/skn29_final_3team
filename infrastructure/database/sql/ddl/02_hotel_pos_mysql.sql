-- 책임: POS source의 빈 MySQL schema와 key/check 계약을 생성한다. 데이터 적재나
-- 특정 시연 row는 포함하지 않고 DDL 오류는 bootstrap 실패로 남긴다.
...
) ENGINE=InnoDB COMMENT='F&B store master';
...
) ENGINE=InnoDB COMMENT='F&B order and payment';
...
) ENGINE=InnoDB COMMENT='F&B order item';

CREATE TABLE IF NOT EXISTS schema_version (version varchar(32) PRIMARY KEY);
-- Version identifies this DDL contract. Dataset/release provenance must be
-- published by the ingestion control plane, never inferred from bootstrap.
