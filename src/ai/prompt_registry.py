...
_PROMPTS = {
    "node1.normalize": PromptRecord(
        "node1.normalize", "PROMPT-v1.28.0", "node1", "development", "base", None,
        "DRAFT-BASE-v0.1",
        ...
        "interpretation_recheck, when supplied, requests one bounded second reading of the original question for exactly its target slot after a selected analysis could not be completed. For period_candidates, re-evaluate temporal evidence compositionally; the recheck does not assert that temporal evidence exists, so if the question truly contains none or remains ambiguous, keep period_candidates empty rather than inventing a boundary. For analysis_operation, repeat the mandatory result-shape pass and return the identical operation as the sole intent_candidates item whenever the complete analysis request or explicit shape evidence determines one. Do not change supported metric, dimension, filter, route, period, or shape decisions outside the requested target unless the original question itself requires the change. "
        ...
        "Set requested_route to the kind of work the question asks for, judged from what the question does with the analysis already under discussion rather than from any fixed vocabulary. Use \"PRESENTATION\" when it asks to see the result that already exists rendered differently and asks for no new measurement, \"REPORT_ACTION\" when it asks to place or record that result into a document or report, including when it asks to prepare the result for use outside this conversation such as for submitting, sharing, approving, or filing it, even when no document is named, and \"ANALYSIS\" when it asks for a measurement that must be computed.Return null when the question does not indicate any of these. Set presentation_type to the rendering the question names: SUMMARY for the narrative summary, KPI for governed metric cards only, TABLE for detail rows, a named chart type for a chart, and FULL for every available summary, KPI, chart, and table view; return null when it names none. Return presentation_type only alongside \"PRESENTATION\" or a question that explicitly asks for a rendering. Neither field grants an action: the server re-checks preconditions and may run a governed analysis instead. "
