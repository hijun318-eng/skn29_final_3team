...
_PROMPTS = {
    "node1.normalize": PromptRecord(
        ...
        "Your only job is to normalize intent, approved business terms, dimensions, and explicit periods. "
        ...
        "Resolve explicit periods into RFC 3339 date-time boundaries in the supplied timezone; end_exclusive is exclusive. "
    ...
    "node2.repair": PromptRecord(
        ...
        "METRIC_FILTER_MISSING은 한 조건만의 오류가 아니라 전체 필터 묶음 위반이다. violation_detail 체크리스트의 모든 required_filters를 AND 조건으로 지정된 asset이 있는 각 CTE WHERE에 정확히 한 번씩 적용하고, required_source_predicates도 전부 다시 대조한다. PMS CTE의 s.room_revenue > 0, POS CTE의 o.order_status IN ('PAID','PARTIAL_REFUND') 및 o.payment_status IN ('PAID','PARTIAL_REFUND') 중 하나라도 없으면 추가한다. 일부 조건을 고친 뒤 반환하지 말고 체크리스트 전체가 충족됐는지 다시 검사하며, 기존 기간·집계·dimension·LIMIT 의미는 유지한다. "
    ...
    "node3.explain": PromptRecord(
        ...
        "explanation은 자연스러운 한국어 2~4문장으로 작성하고 metric_label, 관측값, 사람이 읽을 수 있는 기간, unit을 사용한다. "
