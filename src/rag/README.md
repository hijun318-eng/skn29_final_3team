...
| 구성 | 역할 |
|---|---|
| `EvidenceRouter` | 숫자 질문은 SQL, 정책·매뉴얼은 RAG, 혼합 질문은 두 Tool로 분리 |
...
| `ProcessLocalToolRateLimiter` | 단일 프로세스 호출 제한, 다중 인스턴스 운영 시 공유 저장소 필요 |
...
| 검색 대상 매뉴얼 | `data/rag/manuals/` (개별 PDF 37개) |
| 중복 제외 참고본 | `data/rag/reference/` (통합 PDF 1개) |
...
`data/rag/reference`의 통합본은 개별 매뉴얼과 내용이 중복되므로 기본 검색 대상에 포함하지 않는다. 최신 자료는 모두 보존하되, 기본 적재는 `data/rag/manuals`의 개별 PDF 37개만 사용한다.
...
2. `docker-compose --profile rag up -d --build rag-postgres rag-api`로 서비스와 pgvector를 시작한다.
