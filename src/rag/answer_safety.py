...
class EvidenceSafetyGate:
    _DOMAIN_TERMS = {
        "PRIVACY": ("개인정보", "유출", "노출", "오전달", "잘못 전달"),
        "REPORT": ("보고서", "보고 자료", "지표"),
        "CUSTOMER_SERVICE": ("고객응대", "고객 불만", "민원", "서비스 불만"),
        "ROOM": ("객실", "청결", "욕실", "냄새", "해충", "곰팡이"),
        "RESERVATION": ("예약", "체크인", "체크아웃", "입실", "퇴실", "결제"),
        "REFUND": ("취소", "환불", "보상", "노쇼", "위약금"),
        "FOOD": ("식음", "주방", "음식", "위생"),
        "LEISURE": ("레저", "수영장", "체육", "놀이"),
        "FACILITY": ("시설", "설비", "고장", "장애", "누수", "정전"),
        "SAFETY": ("안전", "사고", "쓰러", "실신", "부상", "화재", "감전", "응급"),
        "PARKING_EVENT": ("주차", "행사", "로비"),
    }
    _STOP_WORDS = {
    ...
    def _is_relevant(
        ...
        query_domains = self._domains(query)
        document_domains = self._domains(text)
        if query_domains and not query_domains.intersection(document_domains):
            return False
        ...
        return bool(query_domains.intersection(document_domains) or lexical_hits or target_present)

    def _rank_key(self, group: list[dict[str, Any]], query: str) -> tuple[float, float, float, str]:
        text = self._group_text(group)
        domain_hits = len(self._domains(query).intersection(self._domains(text)))
        lexical_hits = sum(term in text for term in self._query_terms(query))
        retrieval_score = max(self._scores(group), default=0.0)
        identity = f"{group[0].get('manual_id', '')}:{group[0].get('version', '')}"
        return -float(domain_hits), -float(lexical_hits), -retrieval_score, identity

    @classmethod
    def _domains(cls, text: str) -> set[str]:
        lowered = text.lower()
        return {
            domain for domain, terms in cls._DOMAIN_TERMS.items()
