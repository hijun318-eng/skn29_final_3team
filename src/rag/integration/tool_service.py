...
from .rate_limit import ProcessLocalToolRateLimiter
...
class ConfiguredToolRegistry:
    def __init__(self, registrations: tuple[ToolRegistration, ...]):
        self._registrations = {item.tool_code: item for item in registrations}

    def load(self, tool_codes: tuple[str, ...]) -> dict[str, ToolRegistration]:
        return {
            code: self._registrations[code]
    ...
    def list_callable(self, role: str) -> tuple[ToolRegistration, ...]:
        return tuple(
            item for item in self._registrations.values() if item.callable_by(role)
        )
...
class RegistryToolService:
    ...
    def __init__(
        ...
        rate_limiter: ProcessLocalToolRateLimiter | None = None,
    ) -> None:
        self._registry = registry
        self._handlers = dict(handlers)
        self._rate_limiter = rate_limiter or ProcessLocalToolRateLimiter()

    def list_tools(self, role: str) -> tuple[dict[str, Any], ...]:
        listed: list[dict[str, Any]] = []
        seen: set[str] = set()
        for tool in self._registry.list_callable(role):
            canonical_code = self._canonical_tool_code(tool.tool_code)
            if canonical_code in seen:
                continue
            listed.append(asdict(replace(tool, tool_code=canonical_code)))
