...
class ManualArticleFormatter:
    ...
    def specific_numbers(question: str) -> tuple[int, ...]:
        ...
        risk_classification = (
            "보고" not in text
            and any(term in text for term in ("긴급 장애", "긴급 상황", "중대 사안", "중대 상황", "즉시 위험", "위험"))
            and any(term in text for term in ("기준", "판단", "구분", "분류", "보는", "해당", "어떤 경우"))
        )
        if risk_classification:
            return 8, 3
        if "인계" in text and any(term in text for term in ("종료", "완료", "마무리")):
            return 6, 9
        if any(term in text for term in ("즉시 보고", "바로 보고", "관리자 보고", "책임자 보고", "보고 시점", "보고 상황")):
            return (5,)
        ...
        if any(term in text for term in ("업무 종료", "종료 기준", "완료 기준", "마무리")):
            return (9,)
        if any(term in text for term in ("시작 전", "시작하기 전", "먼저 확인")):
            return (2,)
        if any(term in text for term in ("판단 기준", "처리 기준", "어떤 기준", "기준이 뭐")):
            return (3,)
        if any(term in text for term in ("처리 순서", "처리 절차", "진행 순서")):
            return (4,)
        return ()
