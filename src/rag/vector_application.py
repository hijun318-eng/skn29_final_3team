...
class VectorRagApplication:
    ...
    def search(
        ...
        effective_maximum_chunks = (
            max(3, maximum_chunks_per_document)
            if use_current_utterance
        ...
        if domains and not selected_ids:
            title_prefixes = tuple(
                ...
                for prefix in DOMAIN_DOCUMENT_TITLE_PREFIXES.get(domain, ())
            )
            if title_prefixes:
                selected_ids = tuple(
                    document["manual_id"]
                    for document in self._repository.catalog()
                    if str(document["title"]).startswith(title_prefixes)
                )
    ...
    def answer(self, request_id: str, trace_id: str, query: str, evidence_blocks: list[dict]) -> dict:
        import os
