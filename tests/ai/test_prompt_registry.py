...
class PromptRegistryTests(unittest.TestCase):
    def test_registry_tracks_id_version_environment_and_hash(self):
        ...
        self.assertEqual(
            {
                "node1.normalize": "PROMPT-v1.28.0",
                ...
                "report.assistant.turn": "PROMPT-v1.4.0",
            },
    ...
    def test_each_node_has_a_distinct_role_and_prompt_hash(self):
        ...
        self.assertNotIn(
            assistant_turn.metadata()["hash"],
            {assistant.metadata()["hash"], *(item.metadata()["hash"] for item in prompts.values())},
        )
        self.assertIn("existing_artifact", assistant_turn.text)
        self.assertIn("new_data", assistant_turn.text)
    ...
    def test_node1_prompts_use_temporal_contracts_without_phrase_tables(self):
        ...
        self.assertIn("KPI for governed metric cards only", prompts["node1.normalize"])
        self.assertIn("FULL for every available summary", prompts["node1.normalize"])
