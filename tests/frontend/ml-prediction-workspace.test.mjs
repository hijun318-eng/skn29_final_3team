...
test("ML 예측 결과는 핵심 KPI와 날짜별 누적 전망을 먼저 보여주고 기술 정보만 접어 둔다", async () => {
  ...
  try {
    ...
    assert.match(html, /class="is-cumulative"><strong>70 박<\/strong>/);
    assert.match(html, /class="is-cumulative"><strong>150 박<\/strong>/);
    assert.match(html, /class="is-cumulative"><strong>75%<\/strong>/);
