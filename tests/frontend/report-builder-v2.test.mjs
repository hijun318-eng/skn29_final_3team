...
const floatingPanel = source("app/frontend/src/features/reports/components/ReportFloatingPanel.jsx");
...
assert.match(builder, /const libraryModal = libraryOpen && compactInspector/);
assert.match(builder, /inert=\{inspectorModal \|\| undefined\}/);
assert.match(builder, /className="builder-inspector-scrim"/);
assert.match(builder, /className="builder-library-scrim"/);
assert.match(builder, /aria-modal=\{libraryModal \? "true" : undefined\}/);
assert.match(builder, /document\.body\.style\.overflow = "hidden"/);
assert.match(builder, /libraryTriggerRef\?\.current\?\.focus\?\.\(\)/);
...
assert.match(presentation, /returnFocusRef\.current \|\| triggerRef\.current/);
assert.doesNotMatch(presentation, /document\.addEventListener\("keydown"/);
assert.match(presentation, /ref=\{overlayRef\}/);
assert.match(presentation, /event\.key === "Tab"/);
assert.match(presentation, /querySelectorAll\(FOCUSABLE_SELECTOR\)/);
assert.match(presentation, /element !== overlayRef\.current/);
assert.match(presentation, /element\.inert = true/);
assert.match(presentation, /element\.inert = inert/);
assert.match(presentation, /returnFocusRef\.current = document\.activeElement/);
...
assert.match(toolbar, /role="region" aria-label="보고서 편집 명령"/);
...
assert.match(builderStyles, /\.editor-history-actions button\{[^}]*padding:0!important;[^}]*border-radius:0!important;[^}]*background:transparent!important/);
assert.match(builderStyles, /\.notion-editor-sidebar \.report-artifact-library-add\{height:auto!important;min-height:84px!important;align-items:start!important\}/);
assert.match(builderStyles, /\.notion-editor-sidebar \.report-artifact-library-add b\{white-space:normal\}/);
assert.match(builderStyles, /\.editor-history-actions button:disabled\{[^}]*border-radius:0!important;[^}]*background:transparent!important\}/);
assert.match(builderStyles, /@media\(max-width:900px\)\{[\s\S]*\.editor-command-actions>\*\{flex:0 0 auto\}/);
assert.match(builderStyles, /\.report-builder-heading small\{[^}]*flex:none;[^}]*white-space:nowrap/);
assert.match(builderStyles, /@media\(max-width:900px\)\{[\s\S]*\.editor-command-actions\{[^}]*overflow:visible;[^}]*flex-wrap:wrap/);
assert.match(builderStyles, /\.editor-command-actions>\.primary\{[^}]*position:static;[^}]*margin-left:auto/);
assert.match(builderStyles, /@media\(max-width:600px\)\{[\s\S]*\.editor-history-actions\{display:none\}/);
...
assert.match(appStyles, /\.report-notice-shell\{width:fit-content;max-width:min\(720px,calc\(100% - 32px\)\);min-height:42px;[^}]*background:rgba\(15,25,39,\.96\)/);
...
assert.match(appStyles, /\.theme-light \.report-notice-shell\{[^}]*background:rgba\(255,255,255,\.98\)/);
assert.match(appStyles, /\.theme-light \.report-notice-shell\.error\{[^}]*background:#fff8f7/);
...
assert.match(properties, /className="report-size-preset-select"/);
assert.match(properties, /className="report-properties-advanced"/);
assert.match(properties, /className="report-properties-technical"/);
assert.match(builderStyles, /\.report-properties-advanced>summary,\[data-report-builder="v2"\] \.report-properties-technical>summary\{[^}]*min-height:42px/);
assert.match(appStyles, /@media\(max-width:700px\)\{[^\n]*\.enterprise-reports-list \.legacy-report-row\{width:100%;min-width:0/);
assert.match(appStyles, /\.enterprise-reports-list \.legacy-report-actions\{[^}]*grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/);
assert.match(appStyles, /\[data-report-render-root="screen-preview"\] \.legacy-document-actions>div\{[^}]*flex-wrap:wrap/);
assert.match(appStyles, /@media\(max-width:700px\)\{[^\n]*\[data-report-render-root="screen-preview"\] \.report-finalization-panel\{[^}]*grid-template-columns:minmax\(0,1fr\)/);
...
assert.match(page, /onSelectArtifacts=\{page\.setAssistantArtifacts\}/);
...
assert.match(controller, /reportAssistantSessionStorageKey\(lifecycle\.selectedDefinition\)/);
