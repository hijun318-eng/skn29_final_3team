...
class IntegrationFoundationTest(unittest.TestCase):
    ...
    def test_router_calls_only_the_needed_tool_types(self) -> None:
        self.assertEqual(self.router.decide("이번 달 매출은 얼마인가").route, ToolRoute.SQL_ONLY)
        self.assertEqual(self.router.decide("환불 정책과 처리 절차는").route, ToolRoute.RAG_ONLY)
        self.assertEqual(
            self.router.decide("매출이 왜 줄었고 관련 프로모션 정책은 무엇인가").route,
            ToolRoute.SQL_AND_RAG,
        )
        self.assertEqual(self.router.decide("안녕하세요").route, ToolRoute.GENERAL)
