...
class MlToolIntegrationTest(unittest.TestCase):
    def setUp(self) -> None:
        self.context = IntegrationContext(
            ...
            approved_route=ToolRoute.ML_ONLY,
            router_decision_id="approved-route-ml",
        )
    ...
    def test_ml_and_rag_keep_prediction_and_document_separate(self) -> None:
        ...
        response = coordinator.execute(
            "객실 수요 예측과 관련 지침을 함께 확인해줘",
            replace(
                self.context,
                approved_route=ToolRoute.ML_AND_RAG,
                router_decision_id="approved-route-ml-rag",
            ),
        )
    ...
    def test_ml_route_without_server_receipt_is_rejected(self) -> None:
        with self.assertRaisesRegex(ValueError, "ROUTER_DECISION_REQUIRED"):
            EvidenceRouter().decide(ToolRoute.ML_ONLY, None)
